...
description: Microsoft Teams 开发人员文档术语表
...
| [应用程序 Studio](../concepts/build-and-test/app-studio-overview.md) | 用于开始创建或集成自己的 Microsoft Teams 应用的应用。 现在，它已演变为开发人员门户。 <br> **另请参阅**：[开发人员门户](#d) |
...
| Term | 定义 |
| --- | --- |
| [Blazor](../get-started/get-started-overview.md) | 一个免费的开放源代码 Web 框架，支持发人员使用 C# 和 HTML 创建 Web 应用。 它正在由 Microsoft 进行开发。 |
...
| [位置功能](../concepts/device-capabilities/location-capability.md) | 可与应用集成的设备功能，用于了解应用用户的地理位置，以获得增强的协作体验。 此功能目前仅适用于 Teams 移动客户端。 <br>**另请参阅**：[功能](#c)；[媒体功能](#m)；[设备功能](#d)；[Teams Mobile](#t) |
...
| [消息传递扩展](../messaging-extensions/what-are-messaging-extensions.md) | 消息传递扩展是插入应用内容或对消息执行操作的快捷方式。 无需导航离开会话即可使用消息传递扩展。 <br>**另请参阅**：[搜索命令](#s)；[操作命令](#a) |
...
| [Microsoft Graph 浏览器](../graph-api/proactive-bots-and-messages/graph-proactive-bots-and-messages.md) | Microsoft 365 中数据和智能的网关。 它提供统一的可编程模型，可用于访问 Microsoft 365、Windows 10 和企业移动性 + 安全性中的数据。 |
...
| [预配](../toolkit/provision.md) | 在 Azure 和 Microsoft 365 中为应用创建资源的过程，但不会将任何代码（HTML、CSS、JavaScript 等）复制到资源。 这是进行部署的先决条件。 <br>**另请参阅**：[部署](#d) |
|
...
| [SharePoint 网站集](../sbs-gs-spfx.yml) | SharePoint 应用的集合网站。 在可以在 SharePoint 网站上部署基于 SPFx 的应用之前，需要拥有此网站的管理员帐户。 <br>**另请参阅**：SPFx |
...
| [阶段视图](../sbs-meetings-stage-view.yml) | 一个用户界面组件，可用于呈现在 Teams 中以全屏方式打开并固定为选项卡的内容。可调用该组件以在 Teams 中显示 Web 内容。 请注意，它与会议阶段 *不* 同。 <br>**另请参阅**：[会议阶段](#m) |
...
| [Teams](../overview.md) | Microsoft Teams 是组织的终极消息传递应用。 它是用于实时协作和通信、会议、文件和应用共享的工作区。 |
...
| Term | 定义 |
| --- | --- |
| [网站 URL](../tabs/design/tabs-mobile.md) | 应用清单文件（`websiteUrl`）中的一个属性，该属性将应用链接到组织网站或相关产品的登陆页面。 这是 Teams 移动客户端的必需配置。 <br>**另请参阅**：[应用清单](#a)；[Teams 移动](#t) |
