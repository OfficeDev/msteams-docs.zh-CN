...
1. 若要为应用创建应用包Teams，请选择 App Studio **中的清单编辑器****选项卡**：
    ...
    该示例附带自己的清单，旨在生成项目时生成应用包。 manifest.json 文件位于 清单中的 Visual Studio 下的 ```Microsoft.Teams.Samples.HelloWorld.Web``` 中。
...
1. 使用 **测试和分发** App Studio 在 Teams 中安装Teams。
...
1. 选择 **对话框** 底部的"安装"按钮。
    ...
    你的应用现在可用于Teams。 但是，在用应用 ID 和密码更新托管应用程序环境之前，机器人和消息扩展将不起作用。
