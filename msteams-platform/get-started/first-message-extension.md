...
消息扩展使用 [Bot Framework](https://docs.botframework.com) 允许用户通过对话与您的服务交互。  设置标点文件夹后，你的项目将如下所示：
...
1. 在 Microsoft Teams 中注册"边加载"应用程序。
1. 使用 [Azure Function Core Tools](/azure/azure-functions/functions-run-local?#start) 启动在本地运行的应用程序后端。
1. 启动 ngrok 隧道，以便团队可以与你的应用通信。
1. 启动 Microsoft Teams 时，具有指示 Teams 下载该应用程序的命令。
...
若要在 Teams 中成功运行应用，必须具有允许应用程序旁加载的 Microsoft 365 开发帐户。 有关打开帐户详细信息，请参阅 [先决条件](prerequisites.md#enable-sideloading)。

> [!TIP]
> 在并行加载应用之前，使用 [应用验证工具](https://dev.teams.microsoft.com/appvalidation.html)中的问题。 修复错误以成功并行加载应用。
</details>
...
部署涉及预配活动 Azure 订阅上的资源，以及将应用程序后端和前端代码部署（上传）到 Azure。 后端使用多种 Azure 服务，包括 Azure 应用服务和 Azure Bot 服务。
