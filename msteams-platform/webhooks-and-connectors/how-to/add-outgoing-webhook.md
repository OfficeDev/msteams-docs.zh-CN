...
为确保你的服务仅接收来自实际 Teams 客户端的调用，Teams 在 HTTP `hmac` 授权标头中提供了 HMAC 代码。 始终在你的身份验证协议中包含此代码。
...
来自传出 Webhook 的回复显示在与原始消息相同的回复链中。 当用户执行查询时，Microsoft Teams 将向服务发出同步 HTTP 请求，并且你的代码在连接超时和终止之前有 5 秒时间回复消息。
...
> * 卡片支持格式设置。 有关详细信息，请参阅[使用 Markdown 设置卡片格式](~/task-modules-and-cards/cards/cards-format.md?tabs=adaptive-md%2Cconnector-html#format-cards-with-markdown)。
...
|**示例名称** | **说明** | **.NET** | **Node.js** |
|----------------|------------------|--------|----------------|
| 传出 webhook | 创建将在 Microsoft Teams 中使用的自定义机器人的示例。| [View](https://github.com/OfficeDev/Microsoft-Teams-Samples/tree/main/samples/outgoing-webhook/csharp) | [View](https://github.com/OfficeDev/Microsoft-Teams-Samples/tree/main/samples/outgoing-webhook/nodejs)|
