...
对于桌面，可以使用会议内窗口中的 **“添加应用**”选项在会议期间添加应用 :::image type="icon" source="../assets/icons/add-icon.png" border="false"::: 。
...
1. 进入会议后，从聊天窗口的工具栏中选择所需的应用。
    应用在侧面板或会议内对话框中的 Teams 会议中可见。
1. 在会议中对话框中，输入响应作为反馈。

# <a name="desktop"></a>[桌面设备](#tab/desktop)

:::image type="content" source="~/assets/images/apps-in-meetings/desktop-in-meeting-dialog-view.png" alt-text="桌面会议内视图。":::

# <a name="mobile"></a>[移动设备](#tab/mobile)

在进入会议并从桌面或 Web 添加应用后，应用在“ **应用** ”部分下的移动 Teams 会议中可见。 选择 **“应用** ”以显示应用列表。 用户可以将任何应用作为应用的会议内侧面板启动。
...
若要实现消息扩展，请从 [生成消息扩展](../messaging-extensions/how-to/create-messaging-extension.md)开始，然后继续 [为 Teams 会议创建应用](../apps-in-teams-meetings/API-references.md#meeting-apps-api-references)。

Teams 统一会议应用允许你根据会议中的参与者角色设计应用。
...
用户类型（如租户内、来宾、联合用户或会议中的外部用户）可以在 [会议中执行其中一个参与者角色](#participant-roles-in-a-meeting)。
...
* **联合用户或外部** 用户是另一个组织中受邀加入会议的外部 Teams 用户。 联合用户具有与联合合作伙伴的有效凭据，并由 Teams 授权。 他们无权访问你的团队或组织中的其他共享资源。 来宾访问是外部用户有权访问团队和频道的更好选择。 有关详细信息，请参阅 [管理 Teams 中的外部访问权限](/microsoftteams/manage-external-access)。
    ...
    > [!NOTE]
    > Teams 用户可以在托管会议或与其他组织聊天时添加应用。 当用户加入其他组织托管的会议或聊天时，用户可以使用外部用户共享的应用。 托管用户组织的数据策略以及该用户组织共享的第三方应用的数据共享做法将生效。
...
| 来宾，租户 Azure AD 的一部分 | 允许交互。 不允许创建、更新和删除。 | 不可用 | 不可用 | 允许在会议聊天中进行交互。 | 允许从自适应卡片在会议聊天中进行交互。 | 可用 | 只能在 Teams 桌面客户端和 Teams 移动设备上启动、查看和与会议舞台上的应用交互。 |
| 联合用户，有关详细信息，请参阅 [非标准用户](/microsoftteams/non-standard-users)。 | 在计划的会议中允许交互。 不允许创建、更新和删除。 | 允许交互。 不允许获取、更新和删除。 | 不可用 | 允许在会议聊天中进行交互。 | 允许从自适应卡片在会议聊天中进行交互。 | 不可用 | 只能在 Teams 桌面客户端和 Teams 移动设备上启动、查看和与会议舞台上的应用交互。 |

> [!NOTE]
>
> 调用中应用的各种用户类型的行为与其在计划会议中的行为相同，但以下情况除外：
>
> * 联合用户无法在调用中与选项卡应用交互。
