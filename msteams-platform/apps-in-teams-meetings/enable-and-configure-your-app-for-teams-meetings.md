...
在会议之前，用户可以添加选项卡、机器人和消息传递扩展。 具有组织者和演示者角色的用户可以向会议添加选项卡。
...
1. 选择要添加的应用，并根据需要执行步骤。 应用作为消息传递扩展安装。
...
> [!NOTE]
>
> * 内容气泡在用户可以访问的会议聊天中同时发布自适应卡片或卡片。 当会议或Teams应用最小化时，这有助于用户。
...
当用户处于会议内视图中时，消息传递扩展按预期工作。 用户可以发布撰写消息扩展卡。 会议中的 AppName 是一个工具提示，用于在会议 U 栏中声明应用名称。
