...
> [!NOTE]
> 移动客户端仅支持准备会议和投递会议表面中的选项卡。 在会议中 (会议中的对话和面板) 的会议体验将很快可用
...
✔导出调查结果或轮询的结果应通知用户声明 "结果已成功下载"。

✔要在两个区域中的团队会议中显示的应用程序：
...
✔后会议应用程序方案与当前的会议后体验类似，其中增加了包含在表面中存在的选项卡的好处。 

✔具有独特权限用户可以通过 "团队计划" 窗体上的 " **详细信息** " 选项卡和现有会议中的 "会议 **聊天** " 选项卡，将选项卡库中的应用程序添加到会议。
