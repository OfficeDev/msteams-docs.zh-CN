...
* 中央团队管理所有员工体验。 它能够构建一种虚拟助理体验，并管理核心体验的更新，包括新技能的添加。
...
通过将技能信息嵌入卡片操作有效负载中，可以解决此缺点。 每个技能都应 `skillId` 嵌入卡片  `value` 操作字段中。 必须确保每个卡片操作活动都携带相关的技能信息，并且虚拟助理此信息进行调度。
...
若要将任务模块功能添加到虚拟助理，活动处理程序中还包括两虚拟助理方法： 和 `OnTeamsTaskModuleFetchAsync` `OnTeamsTaskModuleSubmitAsync` 。 这些方法侦听任务模块相关活动，虚拟助理确定与请求关联的技能，以及将请求转发到标识的技能。 
...
消息扩展的命令在应用清单文件中声明。 邮件扩展用户界面由这些命令支持。 若要虚拟助理附加技能为消息传递扩展命令提供电源，虚拟助理自己的清单必须包含这些命令。 你必须将单个技能清单中的命令添加到虚拟助理清单中。 命令 ID 通过分隔符附加该技能的应用 ID，提供有关关联技能的信息 `:` 。
...
用户调用命令后，虚拟助理 可以通过分析命令 ID 来标识关联的技能，通过从命令 ID 中删除额外的后缀来更新活动，然后将其转发到相应的技能 `:<skill_id>` 。 技能的代码不需要处理额外的后缀。 因此，可以避免跨技能的命令 ID 冲突。 通过此方法，所有上下文中技能的所有搜索和操作命令（如 **compose、commandBox** 和 **message）** 都由虚拟助理。
...
以下示例演示如何将"预订-会议室"应用模板转换为 虚拟助理 技能：书籍-会议室是一种 Microsoft Teams，允许用户从当前时间开始快速查找和预留会议室 30、60 或 90 分钟。 默认时间为 30 分钟。 Book-a-room bot 作用域为个人对话或 1：1 对话。 下图显示一个虚拟助理 **具有会议室技能的书籍**：
...
更新活动（如卡片刷新）尚不支持通过虚拟助理 ([github 问题](https://github.com/microsoft/botbuilder-dotnet/issues/3686)) 。 因此，我们已将所有卡刷新呼叫 `UpdateActivityAsync` 替换为发布新卡呼叫 `SendActivityAsync` 。
...
`Book-a-room bot` 专为私人聊天（如个人聊天或 1：1 范围）设计。 由于我们已自定义 虚拟助理 以支持群聊和频道范围，因此必须从频道范围调用 虚拟助理，因此自动程序必须获取相同 `Book-a-room` 范围的活动。 因此 `Book-a-room` ，自动程序会进行自定义以处理这些活动。 你可以找到自动程序的活动处理程序 `OnMessageActivityAsync` `Book-a-room` 的签入方法。
