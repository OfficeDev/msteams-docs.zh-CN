...
| 11/11/2018 | Node JS 和 .NET/C# 入门已更新为使用 Teams 中的 App Studio，并且添加了一个新部分，以在 Azure 中托管基于 Node Teams 应用。 | [使用 C# Microsoft Teams/.NET](~/get-started/get-started-dotnet-app-studio.md)和 App Studio 在 Microsoft Teams 平台上开始使用，使用 Node JS 和[App Studio](~/get-started/get-started-nodejs-app-studio.md)在 Microsoft Teams 平台上入门，在 Azure 中托管 Node [Teams 应用](~/get-started/get-started-nodejs-in-azure.md)|
...
除了新功能之外，我们还提供生产就绪[](samples/app-templates.md)Teams模板，你可以立即部署这些模板或根据需要进行修改。 新添加的模板用星号表示 。
