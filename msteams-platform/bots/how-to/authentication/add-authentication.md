...
<span data-ttu-id="e7d1e-141">资源组和服务计划不是必需的，但它们使您可以方便地释放所创建的资源。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-141">The resource group and the service plan aren't strictly necessary, but they allow you to conveniently release the resources you create.</span></span> <span data-ttu-id="e7d1e-142">这对于保持资源的组织和可管理性来说是很好的做法。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-142">This is good practice for keeping your resources organized and manageable.</span></span>
...
1. <span data-ttu-id="e7d1e-163">在 [**Azure 门户**][azure-portal]的左侧导航窗格中，选择 " **创建资源**"。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-163">In the [**Azure portal**][azure-portal], on the left navigation panel, select **Create a resource**.</span></span>
...
1. <span data-ttu-id="e7d1e-272">如下所示完成表单：</span><span class="sxs-lookup"><span data-stu-id="e7d1e-272">Complete the form as follows:</span></span>
    ...
    1. <span data-ttu-id="e7d1e-273">**名称**。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-273">**Name**.</span></span> <span data-ttu-id="e7d1e-274">输入连接的名称。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-274">Enter a name for the connection.</span></span> <span data-ttu-id="e7d1e-275">您将在文件的 bot 中使用此名称 `appsettings.json` 。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-275">You'll use this name in your bot in the `appsettings.json` file.</span></span> <span data-ttu-id="e7d1e-276">例如 *BotTeamsAuthADv2*。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-276">For example *BotTeamsAuthADv2*.</span></span>
...
### <a name="test-the-connection"></a><span data-ttu-id="e7d1e-293">测试连接</span><span class="sxs-lookup"><span data-stu-id="e7d1e-293">Test the connection</span></span>
...
1. <span data-ttu-id="e7d1e-313">在 c # 中更新 **appsettings.js** ，如下所示：</span><span class="sxs-lookup"><span data-stu-id="e7d1e-313">In C# Update **appsettings.json** as follows:</span></span>
    ...
    - <span data-ttu-id="e7d1e-314">设置 `ConnectionName` 为你添加到 bot 通道注册的标识提供程序连接的名称。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-314">Set `ConnectionName` to the name of the identity provider connection you added to the bot channel registration.</span></span> <span data-ttu-id="e7d1e-315">本示例中使用的名称是 *BotTeamsAuthADv1*。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-315">The name we used in this example is *BotTeamsAuthADv1*.</span></span>
...
1. <span data-ttu-id="e7d1e-343">在 Visual Studio " *解决方案资源管理器* " 中，选择并按住 (或右键单击) 项目名称。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-343">In Visual Studio *Solution Explorer* select and hold (or right-click) the project name.</span></span>
...
## <a name="test-the-bot-using-the-emulator"></a><span data-ttu-id="e7d1e-365">使用模拟器测试机器人</span><span class="sxs-lookup"><span data-stu-id="e7d1e-365">Test the bot using the Emulator</span></span>
...
### <a name="test-the-bot-locally"></a><span data-ttu-id="e7d1e-382">本地测试机器人</span><span class="sxs-lookup"><span data-stu-id="e7d1e-382">Test the bot locally</span></span>
...
1. <span data-ttu-id="e7d1e-398">根据您对仿真程序使用的配置，可以获取以下内容之一：</span><span class="sxs-lookup"><span data-stu-id="e7d1e-398">Depending which configuration you used for the Emulator, you get one of the following:</span></span>
    ...
    <span data-ttu-id="e7d1e-404">以下图像是登录后的 bot UI 的示例：</span><span class="sxs-lookup"><span data-stu-id="e7d1e-404">The following image is an example of the bot UI after you've logged in:</span></span>
...
1. <span data-ttu-id="e7d1e-423">如果需要，请选择适用用户的帐户。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-423">If asked, select the applicable user's account.</span></span>
    ...
    ![已部署身份验证 bot 登录](../../../assets/images/authentication/auth-bot-login-deployed.PNG)<span data-ttu-id="e7d1e-426">.</span><span class="sxs-lookup"><span data-stu-id="e7d1e-426">.</span></span>
...
1. <span data-ttu-id="e7d1e-463">例如，运行 `ngrok http 3978 --host-header=localhost:3978` 。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-463">Run, for example, `ngrok http 3978 --host-header=localhost:3978`.</span></span> <span data-ttu-id="e7d1e-464">根据需要更换端口号。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-464">Replace the port number as needed.</span></span>
<span data-ttu-id="e7d1e-465">这将启动 ngrok 以侦听您指定的端口。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-465">This launches ngrok to listen on the port you specify.</span></span> <span data-ttu-id="e7d1e-466">在 return 中，它为您提供一个外部可寻址的 URL，只要 ngrok 正在运行，它就会有效。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-466">In return, it gives you an externally addressable URL, valid for as long as ngrok is running.</span></span> <span data-ttu-id="e7d1e-467">下面的图像是一个示例：</span><span class="sxs-lookup"><span data-stu-id="e7d1e-467">The following image is an example:</span></span>

    ![团队 bot 应用程序身份验证连接字符串 adv1](../../../assets/images/authentication/auth-bot-ngrok-start.PNG)<span data-ttu-id="e7d1e-469">.</span><span class="sxs-lookup"><span data-stu-id="e7d1e-469">.</span></span>

1. <span data-ttu-id="e7d1e-470">复制转发 HTTPS 地址。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-470">Copy the forwarding HTTPS address.</span></span> <span data-ttu-id="e7d1e-471">它应类似于以下内容： `https://dea822bf.ngrok.io/` 。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-471">It should be similar to the following: `https://dea822bf.ngrok.io/`.</span></span>
1. <span data-ttu-id="e7d1e-472">Append `/api/messages` 获取 `https://dea822bf.ngrok.io/api/messages` 。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-472">Append `/api/messages` to obtain `https://dea822bf.ngrok.io/api/messages`.</span></span> <span data-ttu-id="e7d1e-473">这是在 Microsoft 团队的聊天中，在计算机上本地运行的 bot 的 **消息终结点** ，并可通过 web 访问。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-473">This is the **messages endpoint** for the bot running locally on your machine and reachable over the web in a chat in Microsoft Teams.</span></span>
1. <span data-ttu-id="e7d1e-474">要执行的最后一个步骤是更新已部署的 bot 的消息终结点。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-474">One final step to perform is to update the messages endpoint of the deployed bot.</span></span> <span data-ttu-id="e7d1e-475">在此示例中，我们在 Azure 中部署了机器人。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-475">In the example, we deployed the bot in Azure.</span></span> <span data-ttu-id="e7d1e-476">因此，\* \* 我们将执行以下步骤：</span><span class="sxs-lookup"><span data-stu-id="e7d1e-476">So \*\*let's perform these steps:</span></span>
    1. <span data-ttu-id="e7d1e-477">在浏览器中导航到 [**Azure 门户**][azure-portal]。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-477">In your browser navigate to the [**Azure portal**][azure-portal].</span></span>
    1. <span data-ttu-id="e7d1e-478">选择你的 **Bot 频道注册**。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-478">Select your **Bot Channel Registration**.</span></span>
    1. <span data-ttu-id="e7d1e-479">在左面板中，选择 " **设置**"。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-479">In the left panel, select **Settings**.</span></span>
    1. <span data-ttu-id="e7d1e-480">在右侧面板中的 " **邮件终结点** " 框中，输入 ngrok URL （在我们的示例中） `https://dea822bf.ngrok.io/api/messages` 。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-480">In the right panel, in the **Messaging endpoint** box, enter the ngrok URL, in our example, `https://dea822bf.ngrok.io/api/messages`.</span></span>
1. <span data-ttu-id="e7d1e-481">在本地启动你的 bot，例如在 Visual Studio 调试模式下。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-481">Start your bot locally, for example in Visual Studio debug mode.</span></span>
1. <span data-ttu-id="e7d1e-482">使用 Bot 框架门户的 **测试 Web 聊天**在本地运行时测试机器人。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-482">Test the bot while running locally using the Bot Framework portal's **Test Web chat**.</span></span> <span data-ttu-id="e7d1e-483">与模拟器一样，此测试不允许您访问特定于团队的功能。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-483">Like the Emulator, this test doesn't allow you to access Teams-specific functionality.</span></span>
1. <span data-ttu-id="e7d1e-484">在运行的终端窗口中 `ngrok` ，可以查看 bot 和 web 聊天客户端之间的 HTTP 流量。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-484">In the terminal window where `ngrok` is running you can see HTTP traffic between the bot and the web chat client.</span></span> <span data-ttu-id="e7d1e-485">如果需要更详细的视图，请在浏览器窗口中输入 `http://127.0.0.1:4040` 从以前的终端窗口中获取的。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-485">If you want a more detailed view, in a browser window enter `http://127.0.0.1:4040` you obtained from the previous terminal window.</span></span> <span data-ttu-id="e7d1e-486">下面的图像是一个示例：</span><span class="sxs-lookup"><span data-stu-id="e7d1e-486">The following image is an example:</span></span>

    ![auth bot 团队 ngrok 测试](../../../assets/images/authentication/auth-bot-teams-ngrok-testing.png)<span data-ttu-id="e7d1e-488">.</span><span class="sxs-lookup"><span data-stu-id="e7d1e-488">.</span></span>

> [!NOTE]
> <span data-ttu-id="e7d1e-489">如果您停止并重新启动 ngrok，则 URL 会发生更改。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-489">If you stop and restart ngrok, the URL changes.</span></span> <span data-ttu-id="e7d1e-490">若要在项目中使用 ngrok，并根据所使用的功能，必须更新所有 URL 引用。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-490">To use ngrok in your project, and depending on the capabilities you're using, you must update all URL references.</span></span>
 

## <a name="additional-information"></a><span data-ttu-id="e7d1e-491">其他信息</span><span class="sxs-lookup"><span data-stu-id="e7d1e-491">Additional information</span></span>

### <a name="teamsappmanifestmanifestjson"></a><span data-ttu-id="e7d1e-492">上的 TeamsAppManifest/manifest.js</span><span class="sxs-lookup"><span data-stu-id="e7d1e-492">TeamsAppManifest/manifest.json</span></span>

<span data-ttu-id="e7d1e-493">此清单包含 Microsoft 团队与 bot 连接所需的信息。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-493">This manifest contains information needed by Microsoft Teams to connect with the bot.</span></span>  
...
<span data-ttu-id="e7d1e-494">通过身份验证，团队的行为与其他频道略有不同，如下所述。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-494">With authentication, Teams behaves slightly differently than other channels, as explained below.</span></span>

### <a name="handling-invoke-activity"></a><span data-ttu-id="e7d1e-495">处理调用活动</span><span class="sxs-lookup"><span data-stu-id="e7d1e-495">Handling Invoke Activity</span></span>

<span data-ttu-id="e7d1e-496">**调用活动**将发送到 bot，而不是其他通道使用的事件活动。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-496">An **Invoke Activity** is sent to the bot rather than the Event Activity used by other channels.</span></span>
<span data-ttu-id="e7d1e-497">这是通过 **ActivityHandler**的 classing 完成的。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-497">This is done by sub-classing the **ActivityHandler**.</span></span>

# <a name="cnet"></a>[<span data-ttu-id="e7d1e-498">C#/.NET</span><span class="sxs-lookup"><span data-stu-id="e7d1e-498">C#/.NET</span></span>](#tab/dotnet-sample)

<span data-ttu-id="e7d1e-499">**Bot/DialogBot**</span><span class="sxs-lookup"><span data-stu-id="e7d1e-499">**Bots/DialogBot.cs**</span></span>

[!code-csharp[ActivityHandler](~/../botbuilder-samples/samples/csharp_dotnetcore/46.teams-auth/Bots/DialogBot.cs?range=19-51)]

<span data-ttu-id="e7d1e-500">**Bot/TeamsBot**</span><span class="sxs-lookup"><span data-stu-id="e7d1e-500">**Bots/TeamsBot.cs**</span></span>

<span data-ttu-id="e7d1e-501">如果使用了**OAuthPrompt** ，则必须将*调用活动*转发到对话框。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-501">The *Invoke Activity* must be forwarded to the dialog if the **OAuthPrompt** is used.</span></span>

[!code-csharp[ActivityHandler](~/../botbuilder-samples/samples/csharp_dotnetcore/46.teams-auth/Bots/TeamsBot.cs?range=34-42)]

#### <a name="teamsactivityhandlercs"></a><span data-ttu-id="e7d1e-502">TeamsActivityHandler.cs</span><span class="sxs-lookup"><span data-stu-id="e7d1e-502">TeamsActivityHandler.cs</span></span>
...
# <a name="javascript"></a>[<span data-ttu-id="e7d1e-503">JavaScript</span><span class="sxs-lookup"><span data-stu-id="e7d1e-503">JavaScript</span></span>](#tab/node-js-dialog-sample)

<span data-ttu-id="e7d1e-504">**bot/dialogBot.js**</span><span class="sxs-lookup"><span data-stu-id="e7d1e-504">**bots/dialogBot.js**</span></span>

[!code-javascript[ActivityHandler](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/bots/dialogBot.js?range=4-46)]

<span data-ttu-id="e7d1e-505">**bot/teamsBot.js**</span><span class="sxs-lookup"><span data-stu-id="e7d1e-505">**bots/teamsBot.js**</span></span>

<span data-ttu-id="e7d1e-506">如果使用了**OAuthPrompt** ，则必须将*调用活动*转发到对话框。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-506">The *Invoke Activity* must be forwarded to the dialog if the **OAuthPrompt** is used.</span></span>

[!code-javascript[ActivityHandler](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/bots/teamsBot.js?range=4-33)]

<span data-ttu-id="e7d1e-507">**对话框/mainDialog.js**</span><span class="sxs-lookup"><span data-stu-id="e7d1e-507">**dialogs/mainDialog.js**</span></span>

<span data-ttu-id="e7d1e-508">在对话步骤中，使用 `beginDialog` "" 启动 OAuth 提示，这将要求用户登录。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-508">Within a dialog step, use `beginDialog` to start the OAuth prompt, which asks the user to sign in.</span></span>

- <span data-ttu-id="e7d1e-509">如果用户已登录，则会生成令牌响应事件，而不会提示用户。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-509">If the user is already signed in, this will generate a token response event, without prompting the user.</span></span>
- <span data-ttu-id="e7d1e-510">否则，这将提示用户登录。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-510">Otherwise, this will prompt the user to sign in.</span></span> <span data-ttu-id="e7d1e-511">Azure Bot 服务在用户尝试登录后发送令牌响应事件。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-511">The Azure Bot Service sends the token response event after the user attempts to sign in.</span></span>

[!code-javascript[AddOAuthPrompt](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/dialogs/mainDialog.js?range=50-52)]

<span data-ttu-id="e7d1e-512">在下面的对话框步骤中，检查上一步的结果中是否存在令牌。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-512">Within the following dialog step, check for the presence of a token in the result from the previous step.</span></span> <span data-ttu-id="e7d1e-513">如果不为 null，则用户已成功登录。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-513">If it is not null, the user successfully signed in.</span></span>

[!code-javascript[AddOAuthPrompt](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/dialogs/mainDialog.js?range=50-64)]

<span data-ttu-id="e7d1e-514">**bot/logoutDialog.js**</span><span class="sxs-lookup"><span data-stu-id="e7d1e-514">**bots/logoutDialog.js**</span></span>

[!code-javascript[allow-logout](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/dialogs/logoutDialog.js?range=31-42&highlight=7)]

# <a name="python"></a>[<span data-ttu-id="e7d1e-515">Python</span><span class="sxs-lookup"><span data-stu-id="e7d1e-515">Python</span></span>](#tab/python-sample)

<span data-ttu-id="e7d1e-516">**bot/dialog_bot py**</span><span class="sxs-lookup"><span data-stu-id="e7d1e-516">**bots/dialog_bot.py**</span></span>

[!code-python[ActivityHandler](~/../botbuilder-samples/samples/python/46.teams-auth/bots/dialog_bot.py?range=10-42)]

<span data-ttu-id="e7d1e-517">**bot/teams_bot py**</span><span class="sxs-lookup"><span data-stu-id="e7d1e-517">**bots/teams_bot.py**</span></span>

<span data-ttu-id="e7d1e-518">如果使用了**OAuthPrompt** ，则必须将*调用活动*转发到对话框。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-518">The *Invoke Activity* must be forwarded to the dialog if the **OAuthPrompt** is used.</span></span>

[!code-python[on_token_response_event](~/../botbuilder-samples/samples/python/46.teams-auth/bots/teams_bot.py?range=38-45)]

<span data-ttu-id="e7d1e-519">**对话框/main_dialog py**</span><span class="sxs-lookup"><span data-stu-id="e7d1e-519">**dialogs/main_dialog.py**</span></span>

<span data-ttu-id="e7d1e-520">在对话步骤中，使用 `begin_dialog` "" 启动 OAuth 提示，这将要求用户登录。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-520">Within a dialog step, use `begin_dialog` to start the OAuth prompt, which asks the user to sign in.</span></span>

- <span data-ttu-id="e7d1e-521">如果用户已登录，则会生成令牌响应事件，而不会提示用户。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-521">If the user is already signed in, this will generate a token response event, without prompting the user.</span></span>
- <span data-ttu-id="e7d1e-522">否则，这将提示用户登录。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-522">Otherwise, this will prompt the user to sign in.</span></span> <span data-ttu-id="e7d1e-523">Azure Bot 服务在用户尝试登录后发送令牌响应事件。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-523">The Azure Bot Service sends the token response event after the user attempts to sign in.</span></span>

[!code-python[Add OAuthPrompt](~/../botbuilder-samples/samples/python/46.teams-auth/dialogs/main_dialog.py?range=48-49)]

<span data-ttu-id="e7d1e-524">在下面的对话框步骤中，检查上一步的结果中是否存在令牌。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-524">Within the following dialog step, check for the presence of a token in the result from the previous step.</span></span> <span data-ttu-id="e7d1e-525">如果不为 null，则用户已成功登录。</span><span class="sxs-lookup"><span data-stu-id="e7d1e-525">If it is not null, the user successfully signed in.</span></span>

[!code-python[Add OAuthPrompt](~/../botbuilder-samples/samples/python/46.teams-auth/dialogs/main_dialog.py?range=51-61)]

<span data-ttu-id="e7d1e-526">**对话框/logout_dialog py**</span><span class="sxs-lookup"><span data-stu-id="e7d1e-526">**dialogs/logout_dialog.py**</span></span>
...
> [<span data-ttu-id="e7d1e-527">了解如何通过 Azure Bot 服务添加身份验证</span><span class="sxs-lookup"><span data-stu-id="e7d1e-527">Learn about adding adding authentication via Azure Bot Service</span></span>](https://aka.ms/azure-bot-add-authentication)
