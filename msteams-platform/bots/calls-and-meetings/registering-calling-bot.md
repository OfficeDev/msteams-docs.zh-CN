...
description: 了解如何为用户注册新的音频/视频Microsoft Teams、创建新的机器人或添加呼叫功能，以及添加图形权限。
ms.topic: conceptual
...
* 新版应用清单Teams两个附加设置 `supportsCalling` `supportsVideo`和 。 这些设置包含在清单架构[中，Microsoft Teams](../../resources/schema/manifest-schema.md)。
...
呼叫和联机会议机器人在 manifest.json 中具有以下两个其他设置，这些设置为 Teams 中的机器人启用音频或Teams。
...
1. 添加Teams通道。
1. 选择"**通话** 频道"页面上Teams"选项卡。 选择 **"启用** 呼叫"，然后更新 **Webhook (** ，以便) 接收传入通知的 HTTPS URL 调用 Webhook，例如 `https://contoso.com/teamsapp/api/calling`。 有关详细信息，请参阅 [配置通道](/bot-framework/portal-configure-channels)。
...
对于使用 Microsoft Azure Active Directory (Azure AD) V1 终结点的应用，当应用安装在其组织中时，租户管理员可以使用 [Microsoft Azure](https://portal.azure.com) 门户同意应用程序权限。 或者，可以在应用中提供注册体验，管理员可通过此体验同意你配置的权限。 管理员同意由管理员记录Microsoft Azure Active Directory (Azure AD) ，应用无需再次请求同意即可请求令牌。

你可以依赖管理员在应用程序门户授予Microsoft Azure[权限](https://portal.azure.com)。 更好的选择是使用 V2 终结点为管理员Microsoft Azure Active Directory (Azure AD) 体验`/adminconsent`。 有关详细信息，请参阅 [有关构造管理员同意 URL 的说明](/graph/uth-v2-service#3-get-administrator-consent)。
