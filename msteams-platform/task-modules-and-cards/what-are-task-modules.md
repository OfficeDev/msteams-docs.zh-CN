...
任务模块允许你在 Teams 应用程序中创建模式弹出体验。 在弹出窗口中，可以运行自己的自定义 HTML/JavaScript 代码，显示基于小部件（如 YouTube 或 Microsoft Stream 视频）或 `<iframe>` 显示 [自适应卡片](/adaptive-cards/)。 它们对于启动和完成任务或显示丰富的信息（如视频或 Power BI 仪表板）特别有用。 与选项卡或基于对话的自动程序体验相比，用户启动和完成任务通常更自然。

任务模块基于 Microsoft Teams 选项卡构建;它们实质上是弹出窗口中的一个选项卡。 它们使用相同的 SDK，因此如果你已生成选项卡，则你已经 90% 能够创建任务模块。
...
* **机器人。** 从自动程序 [发送](~/task-modules-and-cards/cards/cards-reference.md) 的卡片上的按钮。 当你不需要频道中的每个人查看你正在使用机器人做什么时，这尤其有用。 例如，让用户在频道中回复投票时，看到创建该投票的记录并没有多大用处。 [此处详细介绍了这一点。](~/task-modules-and-cards/task-modules/task-modules-bots.md)
* **从深层链接的 Teams 外部。** 还可以创建 URL 以从任何位置调用任务模块。 [此处详细介绍了这一点。](#task-module-deep-link-syntax)
...
3. 任务模块的标题在 `title` [TaskInfo](#the-taskinfo-object)对象的 属性中指定。
4. 任务模块的关闭/取消按钮。 如果用户按此键，你的应用将收到一 `err` 个事件，如此处 [所述](~/task-modules-and-cards/task-modules/task-modules-tabs.md#example-submitting-the-result-of-a-task-module)。  (**注意：** 从 bot.) 
5. 如果正在使用 TaskInfo 对象的 属性加载自己的网页，则蓝色矩形是显示 `url` [网页的位置](#the-taskinfo-object)。 下面的任务模块 [大小调整部分提供了更多详细信息](#task-module-sizing) 。
6. 如果你要通过 TaskInfo 对象的 属性显示自适应卡片，则添加填充，否则你需要自己 `card` [处理。](#task-module-css-for-htmljavascript-task-modules) [](#the-taskinfo-object)
7. 自适应卡片按钮将在此处呈现。 如果正在使用自己的页面，则必须创建自己的按钮。
...
`TaskInfo`对象包含任务模块的元数据。 对象定义如下所示。 你必须 **为** 嵌入式 `url` iFrame (定义) ， (定义自适应卡片 `card`) 。

| 属性 | 类型 | 说明 |
| --- | --- | --- |
| `title` | string | 显示在应用名称下方和应用图标右侧 |
