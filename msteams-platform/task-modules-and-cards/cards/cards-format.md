...
卡片类型之间的格式支持不同。 桌面版和移动版客户端以及桌面Microsoft Teams和桌面Teams的卡片呈现可能略有不同。

你可以将内联图像与任意卡片Teams内。 图像的格式可以设置为 、 或 文件，并且不得超过 `.png` `.jpg` `.gif` 1024 像素× 1024 像素或 1 MB。 不支持动态 GIF。 有关详细信息，请参阅 [卡片类型](./cards-reference.md#inline-card-images)。
...
你可以为@mentions和消息传递扩展响应在自适应卡片正文中添加邮件。 若要在@mentions，请遵循与频道和群聊对话中基于消息的提及相同的通知逻辑和 [呈现](../../bots/how-to/conversations/channel-and-group-conversations.md#work-with-mentions)。
...
> [!NOTE]
> * [媒体元素](https://adaptivecards.io/explorer/Media.html)当前在应用平台上的自适应卡片Teams支持。
> * 机器人消息不支持频道和团队提及。
...
Teams平台除了现有提及 ID 外，AAD还允许使用其 AAD 对象 ID 和用户原则名称 (UPN) 来提及用户。 具有自适应卡片的机器人和具有传入 Webhook 的连接器支持两个用户提及的 ID。 
...
除了现有 ID 外，机器人还支持AAD对象 ID 和 UPN 进行用户提及。 在短信、自适应卡片正文和消息传递扩展响应的机器人中提供对两个新 ID 的支持。 机器人支持会话和方案中的提及 `invoke` 设备。 用户与 ID 一起@mentioned活动源通知。 
...
> * 对于使用对象 ID 和 UPN 的用户AAD UI/UX 更改。      
...
若要屏蔽自适应卡片中的信息，请 `style` 添加属性以 **键入** `input.text` ，将其值设置为 **Password**。
...
# <a name="html-format-for-office-365-connector-cards"></a>[连接器卡的 HTML Office 365](#tab/connector-html)
