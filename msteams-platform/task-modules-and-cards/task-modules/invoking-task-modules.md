...
任务模块深层链接是 TaskInfo 对象的序列化，包含以下两个其他详细信息 `APP_ID` （可选 `BOT_APP_ID` ）：
...
| `BOT_APP_ID` | string | 否 | 如果指定了 `completionBotId` 的值，则使用消息将对象 `result` `task/submit invoke` 发送到指定的自动程序。 `BOT_APP_ID` 必须在应用清单中指定为自动程序，即你无法将其发送到任何自动程序。 |
