...
ms.locfileid: "65111876"
---
# <a name="invoke-and-dismiss-task-modules"></a>调用和关闭任务模块

可以从选项卡、机器人或深层链接调用任务模块。 响应可以是 HTML、JavaScript 或自适应卡片。 在如何调用任务模块以及如何处理用户交互的响应方面有很大灵活性。 下表总结了其工作原理：
...
根据调用 `card` 的方式，必须使用自适应卡片或自适应卡片机器人卡片附件，即包装在附件对象中的自适应卡片。
...
| 值 | 类型 | 必需 | 说明 |
| --- | --- | --- | --- |
| `APP_ID` | string | 是 | 调用任务模块的应用的 [ID](~/resources/schema/manifest-schema.md#id)。 如果 `url` 在 URL 中，则 `APP_ID` 清单中的 [validDomains 数组](~/resources/schema/manifest-schema.md#validdomains)必须包含 `url` 的域。 当从选项卡或机器人调用任务模块时，应用 ID 是已知的，这就是它不包含在 `TaskInfo` 中的原因。 |
| `BOT_APP_ID` | string | 否 | 如果指定了 `completionBotId` 的值，则使用 `task/submit invoke` 消息将 `result` 对象发送到指定的机器人。 必须在应用的清单中将 `BOT_APP_ID` 指定为机器人，即不能将其发送给任何机器人。 |
