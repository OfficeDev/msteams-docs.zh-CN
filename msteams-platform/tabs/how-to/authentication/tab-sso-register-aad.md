...
- **单租户或多租户选项**：应用程序将仅在注册它的 Microsoft 365 租户中使用，还是许多 Microsoft 365 租户使用它？ 为一家企业编写的应用程序通常是单租户的;由独立软件供应商编写且由许多客户使用的应用程序需要是多租户，以便每个客户的租户都可以访问该应用程序。
- **应用程序 ID URI**：它是一个全局唯一的 URI，用于标识通过范围为应用的访问公开的 Web API。 它也称为标识符 URI。 应用程序 ID URI 包括应用 ID 和托管应用的子域。 应用程序的域名和为 Azure AD 应用程序注册的域名应相同。 目前，不支持每个应用的多个域。
- **范围**：可以授予授权应用用户或应用访问 API 公开的资源的权限。

> [!NOTE]
>
> - **LOB 应用程序**：组织可以通过 Microsoft Store 提供 LOB 应用程序。 这些应用是组织的自定义应用。 它们在组织或企业内部或特定。
...
## <a name="create-an-app-registration-in-azure-ad"></a>在 Azure AD 中创建应用注册
...
- [若要公开 API](#to-expose-an-api)，请为应用配置范围 (权限) 选项。 你将公开 Web API，并配置应用程序 ID URI。
- [若要配置 API 范围](#to-configure-api-scope)：定义 API 的范围，以及可以同意某个范围的用户。 只能让管理员为特权较高的权限提供许可。
- [若要配置授权的客户端应用程序](#to-configure-authorized-client-application)：为要预先授权的应用程序创建授权的客户端 ID。 它允许应用用户访问已配置) 的应用范围 (权限，而无需任何进一步的同意。 仅预先授权你信任的客户端应用程序，因为应用用户将没有机会拒绝同意。
...
1. 以此处所述的格式输入应用程序 ID URI。
    ...
    - `fully-qualified-domain-name.com`插入 GUID) 之间的`api://` `{AppID}` 和 (。 例如，api://example.com/{AppID}。

    where，
    - `fully-qualified-domain-name.com` 是可从中提供选项卡应用的可读域名。 应用程序的域名和为 Azure AD 应用程序注册的域名应相同。

      如果使用的是隧道服务（如 ngrok），则必须在 ngrok 子域发生更改时更新此值。
    - `AppID` 是注册应用时生成的 (GUID) 的应用 ID。 可以在“ **概述** ”部分中查看它。

    > [!IMPORTANT]
    >
    > - **具有多种功能的应用的应用程序 ID URI**：如果要生成具有机器人、消息传递扩展和选项卡的应用，请输入应用程序 ID URI，因为 `api://fully-qualified-domain-name.com/BotId-{YourClientId}`BotID 是机器人应用 ID。
    >
    > - **域名的格式**：对域名使用小写字母。 请勿使用大写。
    >
    >   例如，若要创建资源名称为“demoapplication”的应用服务或 Web 应用：
    >
    >   | 如果使用了基本资源名称 | URL 将为... | 格式受支持... |
    >   | --- | --- | --- |
