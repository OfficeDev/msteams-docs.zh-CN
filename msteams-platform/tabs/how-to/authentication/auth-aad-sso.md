...
ms.locfileid: "59475683"
---
# <a name="single-sign-on-sso-support-for-tabs"></a>单一登录 (SSO) 选项卡支持

用户通过Microsoft Teams、学校或 Microsoft 帐户（Office 365、Outlook等）登录登录。 可以通过允许单一登录在桌面或移动客户端上授权Teams选项卡或任务模块来利用此优势。 如果用户登录一次，则当自动登录时，他们不必再次在另一台设备上登录。 此外，你的访问令牌会被预取以提高性能和加载时间。
...
> **快速入门**  
>
> 开始使用选项卡 SSO 的最简单路径是使用 Teams toolkit for Visual Studio Code。 有关详细信息，请参阅[SSO with Teams toolkit 和 Visual Studio Code for tabs](../../../toolkit/visual-studio-code-tab-sso.md)
...
> [!NOTE]
> 仅在同意一组有限的用户级 API（即电子邮件、配置文件、offline_access `getAuthToken()` 和 OpenId）时有效。 它不能用于进一步Graph范围，如 `User.Read` 或 `Mail.Read` 。 有关建议的解决方法，请参阅获取具有 Graph[权限的访问令牌](#get-an-access-token-with-graph-permissions)。
...
1. 通过选择Teams"添加范围"按钮来预授权用户，在打开的面板中，输入"access_as_user"作为"**范围****名称"。**

> [!NOTE]
> 您必须了解一些重要限制：
>
> * 仅支持用户Graph API 权限，即电子邮件、配置文件、offline_access、OpenId。 如果必须有权访问或 等Graph访问令牌，请参阅获取具有 Graph `User.Read` `Mail.Read` [权限的访问令牌](#get-an-access-token-with-graph-permissions)。
> * 应用程序的域名与为 AAD 应用程序注册的域名相同，这一点很重要。
> * 目前不支持每个应用多个域。
> * 对于新应用程序， `accessTokenAcceptedVersion` `v2` 用户必须设置为 。
...
1. 在 **"注册应用程序"** 页中，输入以下值：
    ...
    * 保留“重定向 URI”为空。
    3. 选择“注册”。
1. 在概述页面上，复制并保存应用程序 (**客户端) ID**。 在更新应用程序清单时，稍后Teams该清单。
1. 在“**管理**”下，选择“**公开 API**”。
...
1. 选择“**添加作用域**”。 在打开的面板中，输入 **access_as_user** 作为 **范围名称**。
...
1. 导航到 **"API 权限"。** 选择 **"添加**  >  **Microsoft Graph** 委派权限"，然后从 API 中添加以下  >  Graph权限：
    * 默认情况下启用 User.Read
    * 电子邮件
    * offline_access
    * OpenId
...
> [!NOTE]
>
> * ¹ 如果你的 AAD 应用在 Teams 中提出身份验证请求的租户中注册，则不能要求用户同意并获取访问令牌。 只有在 AAD 应用注册到其他租户时，用户才同意这些权限。
...
> * 如果未在返回的访问令牌中 (UPN) 用户主体名称，可以在 AAD 中将其添加为可选声明。 [](/azure/active-directory/develop/active-directory-optional-claims)
...
我们的 SSO 当前实现仅授予用户级别权限的许可，这些权限不能用于进行Graph调用。 若要获取执行 Graph 调用所需的 (作用域) ，SSO 解决方案必须实现自定义 Web 服务，以交换从 Teams JavaScript SDK 获取的令牌，以获取包含所需范围的令牌。 这是使用 AAD 的代表 [流完成的](/azure/active-directory/develop/v1-oauth2-on-behalf-of-flow)。
...
1. 使用 检索到的令牌必须使用 AAD 代表流在服务器端进行交换，才能访问其他 Graph `getAuthToken()` API。 [](/azure/active-directory/develop/v2-oauth2-on-behalf-of-flow) 确保对此交换使用 v2 Graph终结点。
