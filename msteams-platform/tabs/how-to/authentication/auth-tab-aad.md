...
你可能希望在 Teams 应用中使用许多服务，其中大多数服务都需要身份验证和授权才能访问该服务。 这些服务包括 Facebook、Twitter 和 Teams。 Teams 用户配置文件信息存储在使用 Microsoft Graph 的 Azure AD 中，本文将重点介绍如何使用 Azure AD 进行身份验证以获取对此信息的访问权限。
...
本文中的代码来自 Teams 示例应用，即 [Microsoft Teams 选项卡身份验证示例（节点）](https://github.com/OfficeDev/microsoft-teams-sample-complete-node)。 它包含一个静态选项卡，用于请求 Microsoft Graph 的访问令牌并显示来自 Azure AD 的当前用户的基本个人资料信息。
...
身份验证流应由用户操作触发。 你不应自动打开身份验证弹出窗口，因为这可能会触发浏览器的弹出窗口阻止程序并对用户产生混淆。
...
与大多数身份提供程序一样，Azure AD 不允许将其内容放置在 iframe 中。 这意味着需要添加一个弹出页来托管身份提供程序。 在以下示例中，此页面是 `/tab-auth/simple-start`。 选择按钮时，可使用 Microsoft Teams 客户端 SDK 的 `microsoftTeams.authenticate()` 函数启动此页面。
...
当弹出页 (`/tab-auth/simple-start`) 显示时，将运行以下代码。 此页面的主要目标是重定向到身份提供程序，以便用户可以登录。 可以使用 HTTP 302 在服务器端执行此重定向，但在本例中，此重定向是在客户端使用对 `window.location.assign()` 的调用来完成的。 这还允许使用 `microsoftTeams.getContext()` 来检索提示信息，这些提示信息可以传递给 Azure AD。
...
* 有关生成身份验证请求和 URL 的帮助，请参阅[获取用户上下文信息](~/tabs/how-to/access-teams-context.md)。 例如，可以将用户的登录名用作 Azure AD登录的 `login_hint` 值，这意味着用户只需输入更少的内容。 请记住，不应直接将此上下文用作身份证明，因为攻击者可能会在恶意浏览器中加载页面，并向其提供所需的任何信息。
...
* `state` 参数用于确认调用回调 URI 的服务就是你调用的服务。 如果回调中的 `state` 参数与在调用期间发送的参数不一致，则返回调用不会进行验证，并且应终止。
* 无需在应用的 manifest.json 文件的 `validDomains` 列表中包含身份提供程序的域。

## <a name="the-callback-page"></a>回调页

在上一节，你调用了 Azure AD 授权服务并传入了用户和应用程序信息，以便 Azure AD 可以向用户呈现其自身的整体授权体验。 你的应用无法控制此体验中发生的情况。 它只知道当 Azure AD 调用你提供的回调页时返回的内容 (`/tab-auth/simple-end`)。

在此页面中，你需要根据 Azure AD 返回的信息判断成功还是失败，并调用 `microsoftTeams.authentication.notifySuccess()` 或 `microsoftTeams.authentication.notifyFailure()`。 如果登录成功，则你将有权访问服务资源。
