...
> [!Note]
> 若要在移动客户端上对选项卡进行身份验证，需要确保使用的是 JavaScript SDK 版本 1.4.1 Teams更高版本。

你可能希望在你的 Teams 应用中使用许多服务，其中大多数服务都需要进行身份验证和授权才能访问该服务。 服务包括 Facebook、Twitter 以及当然Teams。 Teams用户使用 Microsoft Graph 将用户配置文件信息存储在 Azure Active Directory (Azure AD) 中Graph本文将重点介绍使用 Azure AD 进行身份验证，以便访问此信息。
...
本文中的代码来自 Node Teams 中的 Microsoft Teams[示例 (应用程序](https://github.com/OfficeDev/microsoft-teams-sample-complete-node)) 。 它包含一个静态选项卡，用于请求 Microsoft Graph并显示 Azure AD 中当前用户的基本个人资料信息。
...
* 尽管选项卡上下文提供了有关用户的有用信息，但请勿使用此信息对用户进行身份验证，无论是作为选项卡内容 URL 的 URL 参数获取还是通过调用 Microsoft Teams 客户端 `microsoftTeams.getContext()` SDK 中的 函数。 恶意参与者可能会使用自己的参数调用您的选项卡内容 URL，模拟 Microsoft Teams 的网页可以在 iframe 中加载您的选项卡内容 URL，并返回其自己的数据 `getContext()` 到 函数。 你应该将选项卡上下文中的标识相关信息视为提示，并验证这些信息，然后再使用。
