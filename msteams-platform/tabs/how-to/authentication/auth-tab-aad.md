...
你可能想要在 Teams 应用中使用许多服务，并且这些服务中的大多数都需要身份验证和授权才能访问该服务。 服务包括 Facebook、Twitter 和 Teams。 Teams Microsoft Azure AD中存储用户配置文件信息Graph本文将重点介绍使用 Azure AD 访问此信息的身份验证。
...
本文中的代码来自 Node Teams 中的Microsoft Teams[身份验证示例 (应用](https://github.com/OfficeDev/microsoft-teams-sample-complete-node)) 。 它包含一个静态选项卡，用于请求 Microsoft Graph访问令牌，并显示当前用户从 Azure AD。
...
有关[在将](~/concepts/authentication/configure-identity-provider.md) OAuth 2.0 回调重定向 URL (用作标识提供程序) 配置 Azure AD 的详细信息，请参阅主题配置标识提供程序。
...
Azure AD大多数标识提供程序一样，不允许将其内容放置在 iframe 中。 这意味着您需要添加一个弹出窗口来承载标识提供程序。 在下面的示例中，此页面为 `/tab-auth/simple-start`。 `microsoftTeams.authenticate()`选择按钮后，Microsoft Teams客户端 SDK 的 函数启动此页面。
...
* 尽管选项卡上下文提供了有关用户的有用信息，但请勿使用此信息对用户进行身份验证，无论是作为选项卡内容 URL `microsoftTeams.getContext()` 的 URL 参数获取还是通过调用 Microsoft Teams 客户端 SDK 中的 函数。 恶意参与者可能会使用自己的参数调用您的选项卡内容 URL，模拟 Microsoft Teams 的网页可以在 iframe `getContext()` 中加载您的选项卡内容 URL，并自行将数据返回到 函数。 你应该将选项卡上下文中的标识相关信息视为提示，并验证这些信息，然后再使用。
