...
有关[将](~/concepts/authentication/configure-identity-provider.md) OAuth 2.0 回调重定向 URL 配置为标识提供程序 () 配置 OAuth 2.0 Azure AD的详细信息，请参阅主题。
...
Azure AD大多数标识提供程序一样，不允许其内容放置在 iframe 中。 这意味着您需要添加一个弹出窗口来承载标识提供程序。 在下面的示例中，此页面为 `/tab-auth/simple-start`。 `microsoftTeams.authenticate()`选择按钮后，Microsoft Teams客户端 SDK 的 函数启动此页面。
