...
---
# <a name="tabs-link-unfurling-and-stage-view"></a>选项卡链接取消展开和阶段视图

> [!NOTE]
> 此功能仅在公共 [开发人员预览版中](../resources/dev-preview/developer-preview-intro.md) 可用。

阶段视图是一个新的 UI 组件，它允许你呈现在屏幕中以全屏Teams固定为选项卡的内容。
...
阶段视图是一个全屏 UI 组件，你可以调用它来显示 Web 内容。 现有链接取消点击服务已更新，以便它可用于使用自适应卡片和聊天服务将 URL 转换为选项卡。 当用户在聊天或频道中发送 URL 时，该 URL 将取消展开到自适应卡片。 用户可以 **选择卡片中的** "视图"，并直接从"阶段视图"将内容固定为选项卡。
...
|阶段视图|任务模块|
|:-----------|:-----------|
|当您具有要向用户显示的丰富内容（如页面、仪表板、文件等）时，阶段视图非常有用。 它提供了最大的空间，有助于在全屏画布中呈现内容。|[任务](../task-modules-and-cards/task-modules/task-modules-tabs.md) 模块对于显示需要用户注意的消息或收集移动到下一步所需信息特别有用。|
  
## <a name="invoke-the-stage-view"></a>调用阶段视图
...
当用户在桌面客户端上Teams URL 时，将调用自动程序并返回自适应卡片，并可选择在[](../task-modules-and-cards/cards/cards-actions.md)阶段中打开 URL。 启动阶段并传入后， `tabInfo` 你可以添加将阶段固定为选项卡的能力。  
...
1. 当用户选择"查看 **"时**，机器人会收到 `invoke` 一个请求。 请求类型为 `composeExtension/queryLink` 。
1. `invoke` 来自自动程序的响应包含包含类型为的 `tab/tabInfoAction` 自适应卡片。
1. 机器人使用代码 `200` 进行响应。
...
若要通过选项卡中的深层链接调用阶段视图，必须在 API 中包装深层链接 `microsoftTeams.executeDeeplink(url)` URL。 深度链接还可以通过卡片 `OpenURL` 中的操作传递。
