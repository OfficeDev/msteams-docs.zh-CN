...
---
# <a name="build-your-first-channel-and-group-tab-for-microsoft-teams"></a>为 Microsoft Teams 生成你的第一个频道和组选项卡
...
* 使用适用于 Microsoft Teams 代码的 Microsoft Teams Toolkit创建Visual Studio项目。
* 了解与通道选项卡相关的应用配置和基架。
* 创建选项卡内容和选项卡配置。
* 在团队中生成并运行应用进行测试。
...
Microsoft Teams Toolkit可帮助你配置应用并设置与频道和组选项卡相关的基架。 它还包含显示"Hello， World！"的基本配置页和内容页。 消息。

**创建应用项目**

1. 转到"Visual Studio代码"，然后选择 **左侧活动栏上的"Microsoft** :::image type="icon" source="../assets/icons/vsc-toolkit.png"::: Teams"。
1. 当系统提示你登录时，使用 Microsoft 365 开发帐户登录。
...
* **应用基架**：应用基架提供在 Teams 中呈现频道选项卡所需的组件。 不过，你可以处理很多项目，但现在让我们关注以下内容：
...
1. 使用 **F5** 键在 Visual Studio Code 中打开 Teams Web 客户端。
1. 通过 () 在 Teams 中显示你的应用内容，添加可信赖 `localhost` 的外接程序：

   1. 默认情况下，在 Google Chrome (打开一个新选项卡，) **F5** 键打开该选项卡。
   1. 打开 `https://localhost:3000/tab` 并继续执行页面。

1. 选择 **"添加到团队"** 或"添加到聊天"，并找到可用于从 Teams 中的模式进行测试的频道或聊天。
1. 选择 **"设置选项卡"。** 配置页以模式显示。
...
* [生成并运行你的第一个 Microsoft Teams 应用](../build-your-first-app/build-and-run.md) 
