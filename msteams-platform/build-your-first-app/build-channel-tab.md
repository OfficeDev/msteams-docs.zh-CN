...
在本教程中，你将生成基本频道 *选项卡 (也称为* 组选项卡 *) ，* 它是团队频道或聊天的全屏页面。 与个人选项卡不同，用户可以配置此类选项卡的某些方面 (例如，重命名该选项卡，以便其频道) 。

## <a name="your-assignment"></a>您的工作分配

在不久之前，你的组织创建了一个 Teams 应用，该应用使用选项卡在技术支持 (HR 等部门显示重要的) 。 但是，由于它是个人选项卡，每个用户都必须安装该选项卡，以查看它，并且采用低于预期。 换句话说，太多工作人员仍不知道如何联系技术支持。
...
1. 输入 Teams 应用的名称。  (这是应用的默认名称，也是本地计算机上应用项目目录的名称。) 选择组或 Teams 频道 **选项卡**。
...
应用基架提供在 Teams 中呈现频道选项卡的组件。 你可以处理很多项目，但现在你只需关注以下内容：

* 位于项目目录中 `src/components` 的两个文件：
  * `Tab.js` 用于呈现选项卡的内容页。
...
复制以下代码段并更新与您的组织相关的信息，或者出于时间考虑，按如下所示使用代码。
...
将以下规则 (也位于) ，因此无论使用哪个主题，电子邮件链接 `App.css` `src/components` 都更易于阅读。
...
频道或聊天中的每个选项卡都有一个配置页面，即至少具有一个设置选项的模式，在用户添加应用时显示此选项。 默认情况下，配置页会询问用户是否要在安装选项卡时通知频道或聊天。

将一些自定义内容添加到配置页面。 转到项目的目录，打开 ，并更新 (中的占位符内容，如以下示例 `src/components` `TabConfig.js` `return()`) 。
...
> [!TIP]
> 至少，请在此页面上提供有关你的应用的一些简短信息，因为这可能是用户第一次了解它。 还可以包括自定义配置选项或身份验证 [工作流](../tabs/how-to/authentication/auth-aad-sso.md)，这在选项卡配置页面上很常见。

## <a name="5-provide-a-suggested-tab-name"></a>5. 提供建议的选项卡名称

添加通道选项卡时，默认情况下，应用名称 (，例如，第一 **个应用**) 。

这可能没有问题，具体取决于你调用你的应用的名称，但你可能希望提供一个在组协作上下文中更有意义的名称 (例如，团队联系人) 。 

1. 在 `TabConfig.js` 中，转到 `microsoftTeams.settings.setSettings` 。
2. 使用 `suggestedDisplayName` 要默认显示的选项卡名称添加 属性。 
...
你的应用已准备好在 Teams 中进行测试。 为此，你必须具有允许应用旁加载的帐户。  (如果你不确定拥有该帐户，请了解如何获取 Teams 开发 [帐户](../build-your-first-app/build-first-app-overview.md#set-up-your-development-account).) 
...
1. 返回到 Teams。 在模式中，选择"**添加到团队"** 或"添加到聊天"，然后找到可用于测试的频道或聊天。
1. 选择 **"设置选项卡"。** 配置页以模式显示。<br/>
...
* [创建不带工具包的选项卡](../tabs/quickstarts/create-channel-group-tab-node-yeoman.md)

## <a name="next-lesson"></a>下一课程

了解如何生成用于协作的选项卡。 想要尝试构建不同类型的 Teams 应用？
