...
添加到团队的机器人将成为另一个团队成员，@mentioned对话的一部分。 事实上，机器人仅在收到@mentioned接收消息，因此频道上的其他对话不会发送给机器人。

组或频道中的机器人应提供与所有成员相关且合适的信息。 尽管机器人当然可以提供与体验相关的任何信息，但请记住，与它的对话对所有人都可见。 因此，组或频道中出色的机器人应该为所有用户增加价值，并且当然不会无意中共享更适合于一对一对话的信息。

与现在一样，自动程序可能在所有范围内完全相关，无需额外工作。 在这种情况下Microsoft Teams自动程序在所有范围内都运行，但应确保自动程序在选择支持的任何作用域 (用户) 价值。 有关范围详细信息，请参阅应用程序中[Microsoft Teams](~/concepts/build-and-test/app-studio-overview.md)。

开发在组或频道中工作的自动程序使用与个人对话相同的许多功能。 有效负载中的其他事件和数据Teams组和频道信息。 以下各节介绍了这些差异以及常见功能的主要差异。

### <a name="creating-messages"></a>创建邮件

有关在频道中创建消息的机器人详细信息，请参阅 [针对](~/resources/bot-v3/bot-conversations/bots-conv-proactive.md)机器人的主动消息传递，以及专门 [创建频道对话](~/resources/bot-v3/bot-conversations/bots-conv-proactive.md#creating-a-channel-conversation)。
...
* `conversation.id` 回复链 ID，由通道 ID 以及回复链中第一封邮件的 ID 组成。
* `conversation.isGroup` 适用于 `true` 频道或群聊中的聊天机器人消息。
* `conversation.conversationType``channel`或 `groupChat` 。
* `entities` 可以包含一个或多个提及。 有关详细信息，请参阅 [提及](#-mentions)。

### <a name="replying-to-messages"></a>回复邮件

若要回复现有邮件，请调用 [`ReplyToActivity`](/bot-framework/dotnet/bot-builder-dotnet-connector#send-a-reply) .NET [`session.send`](/bot-framework/nodejs/bot-builder-nodejs-use-default-message-handler) 或 Node.js。 Bot Builder SDK 处理所有详细信息。
...
在频道中，回复消息将显示为对启动回复链的回复。 包含 `conversation.id` 通道和顶级消息 ID。 尽管 Bot Framework 负责处理详细信息，但 `conversation.id` 可以根据需要缓存该对话线程的将来回复。

### <a name="best-practice-welcome-messages-in-teams"></a>最佳做法：欢迎邮件Teams

首次将机器人添加到组或团队时，向所有用户发送向机器人介绍的欢迎消息通常很有用。 欢迎消息应提供自动程序功能和用户优势的说明。 理想情况下，消息还应包含用户与应用交互的命令。 为此，请确保自动程序在 `conversationUpdate` `teamsAddMembers` 对象中以 eventType 响应 `channelData` 消息。 请确保 ID 是 `memberAdded` 机器人的应用 ID 本身，因为在将用户添加到团队时将发送相同的事件。 有关详细信息 [，请参阅团队成员或](~/resources/bot-v3/bots-notifications.md#team-member-or-bot-addition) 机器人添加。

添加自动程序时，你可能还希望向团队的每个成员发送个人邮件。 为此，可以提取 [团队名单](~/resources/bot-v3/bots-context.md#fetch-the-team-roster) ，并给每个用户发送一条 [直接消息](~/resources/bot-v3/bot-conversations/bots-conv-proactive.md)。

建议自动程序 *在* 下列情况下不要发送欢迎消息：

* 团队是一个 (明显具有主观性，例如，超过 100 个成员) 。 您的自动程序可能会被视为"spa一"，添加它的人可能会收到投诉，除非您向看到欢迎消息的每个人清楚地传达了机器人的价值主张。
...
由于组或频道中的自动程序仅在邮件中提到 ("@*botname*") 时做出响应，因此组频道中的自动程序接收的每封邮件都包含其自己的名称，因此必须确保邮件分析可以处理该名称。 此外，机器人可以分析提及的其他用户，并提及用户作为消息的一部分。

### <a name="retrieving-mentions"></a>检索提及

在有效负载中的 `entities` 对象中返回提及，并且包含用户的唯一 ID，并且在大多数情况下，还包含提及的用户名称。 可以通过调用 . `GetMentions` NET 的 Bot Builder SDK 中的 函数来检索邮件中提及的所有内容，该函数将返回对象 `Mentioned` 数组。
...
> [!NOTE]
> 您还可以使用 Teams 扩展`GetTextWithoutMentions`函数，该函数去除所有提及，包括自动程序。
