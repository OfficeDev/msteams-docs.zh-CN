...
Teams 清单介绍应用如何集成到 Microsoft Teams 产品中。 清单必须符合托管在 [`https://developer.microsoft.com/json-schemas/teams/v1.11/MicrosoftTeams.schema.json`]( https://developer.microsoft.com/json-schemas/teams/v1.11/MicrosoftTeams.schema.json)的架构。 还支持以前的版本 1.0、1.1,..., 和 1.11（在 URL 中使用"v1.x"）。
...
> [!NOTE]
>  searchUrl 功能不适用于第三方开发人员。
> 如果选项卡需要上下文相关信息来显示相关内容或启动身份验证流，请参阅[ Microsoft Teams 选项卡的"获取上下文](../../tabs/how-to/access-teams-context.md)。
...
|`initialRun`|boolean|||布尔值指示命令最初是否在没有参数时运行。 默认为 **false**。|
...
|`parameters.inputType`|string|128 个字符||定义任务模块上显示的控件类型。 `fetchTask: true` `text, textarea, number, date, time, toggle, choiceset`之一。|
...
指示使用或不使用选项卡标题栏呈现个人应用的位置。 默认为 **false**。
