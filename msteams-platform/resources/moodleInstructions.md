...
>
> * 使用 Microsoft Azure Active Directory (Azure AD) 自动注册 Your Moodle [服务器](https://azure.microsoft.com/services/active-directory/)。
>
...
1. 下载和保存将 [连接 和 Microsoft 365](https://moodle.org/plugins/auth_oidc) [集成](https://moodle.org/plugins/local_o365)插件保存到本地计算机。
...
1. 以管理员角色登录到你的一台用于你的你的 Your 将  设置[从](https://docs.moodle.org/22/en/Settings_block)左侧导航面板中的"网站管理"。
...
1. 从 **左侧导航面板** 中选择"网站管理"以返回到管理员仪表板。 向下滚动到"**本地插件"** 并选择"Microsoft 365 **集成"** 链接。
    ...
    > [!IMPORTANT]
    >
    > * 在Microsoft 365返回到这组页面时，请让 Your Microsoft 365 Pluginle Plugins 配置页面在单独的浏览器选项卡中打开。  
    >
    > * 如果你没有现有的一个一流网站，请转到 Azure 存储库上的 [设置](https://github.com/azure/moodle) ，并快速部署一个一个一流实例并根据需要自定义它。

## <a name="2-configure-the-connection-between-the-microsoft-365-plugins-and-azure-ad"></a>2. 配置应用程序插件Microsoft 365应用程序Azure AD
...
使用生成的 和 `AppID` `Key` 在 Microsoft 365 Plugins 设置页中，使用 Azure AD 配置你的一个将你的一个Azure AD。
...
1. 执行 PowerShell 脚本：
    ...
    1. 输入应用程序的名称，Azure AD，例如，将一个 Pluginle 或一个 Pluginle 插件。
    1. 输入 Your Your Moodle 服务器的 URL。
    1. 复制 **脚本生成的 (`AppID`)** **和应用程序`Key`** () 应用程序 ID，并保存它们。
...
1. 在用户与用户之间同步Azure AD。 首先，请执行以下操作：

    > [!NOTE]
    > 根据你的环境，你可以在此阶段选择不同的选项。

1. 在用户与用户之间同步Azure AD。 根据你的环境，你可以在此阶段选择不同的选项。 首先，请执行以下操作：
    1. 切换到"同步 **设置选项卡**。

    1. 在 **"将用户与Azure AD** 同步"部分中，选中适用于你的环境的复选框。 您必须选择以下选项：  

        ✔ 在将 Accounts 中的用户创建在一Azure AD。

        ✔ 为 In Azure AD 中的用户更新在在一起的所有Azure AD。

    1. 在"**用户创建限制**"部分，可以设置筛选器来限制Azure AD用户同步到用户数。
    1. "**用户字段映射**"部分允许您自定义Azure AD到一个"用户个人资料"字段映射。
    1. 在 **"Teams同步**"部分，可以选择自动创建现有一些或全部的 Teams 课程的组，例如团队。

13. 若要验证 [cron](https://docs.moodle.org/310/en/Cron) 作业并手动运行它们以便首次运行，请选择"将用户与作业同步"部分中的"计划任务管理 **Azure AD链接。** 这会将您带至 **"计划任务"** 页。

    1. 向下滚动并查找"同步 **用户Azure AD** 作业并选择"**立即运行"**。
...
在资源部署到 Azure 后，必须使用消息终结点Microsoft 365一个设置一个与一个用户一起配置一个一个与一个"将一个"的"设置"插件。 你必须从 Azure 中的机器人获取终结点：
...
1. 在将"Team **设置**"页面打开Microsoft 365的"Team Microsoft 365"页面。
...
1. 在"在"在Teams **，** 从你的"Microsoft 365 Plugins 配置"页面打开"一个"一页。"

1. 将应用 ID 粘贴到 Microsoft Teams 应用 ID 字段中，并保存更改。

同步一个一个一流课程后，Microsoft Teams 会自动在团队中安装一个"一线"应用，在 Teams 的"常规"频道中创建一个"一般"选项卡，并配置该选项卡以包含从中同步它的"一流"课程的课程页面。 你现在可以直接从学校开始使用你的Microsoft Teams。
