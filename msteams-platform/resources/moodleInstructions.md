...
> [!NOTE]
> 为了帮助 IT 管理员轻松设置一系列 Teams 和集成，Microsoft 365更新 Open-source Microsoft 365 Plugins for the following：
>
> * 使用 Microsoft Azure Active Directory (Microsoft Azure Active Directory (Azure AD [) ) ](https://azure.microsoft.com/services/active-directory/)自动注册 Your Your Moodle 服务器。
>
...
* Microsoft Azure Active Directory (Azure AD) 管理员凭据。
...
1. 下载并保存将 YourPressle [OpenID 连接](https://moodle.org/plugins/auth_oidc)和 Microsoft 365 [集成](https://moodle.org/plugins/local_o365)插件保存到本地计算机。
...
1. 以管理员角色登录到 Your Moodle server，然后从位于左侧导航面板的 设置 [块](https://docs.moodle.org/22/en/Settings_block)中选择"网站管理"。
...
## <a name="2-configure-the-connection-between-the-microsoft-365-plugins-and-microsoft-azure-active-directory-microsoft-azure-active-directory-azure-ad"></a>2. 配置Microsoft 365插件Microsoft Azure Active Directory (Microsoft Azure Active Directory (Azure AD) ) 

您必须配置应用程序插件Microsoft 365应用程序Microsoft Azure Active Directory (Azure AD) 。

### <a name="requisites"></a>先决条件

使用 PowerShell 脚本将一个 Microsoft Azure Active Directory (Azure AD) 注册为应用程序中。 该脚本设置以下内容：

* 适用于 Microsoft Azure Active Directory (Azure AD) 租户的新 Microsoft 365 应用程序，由 Microsoft 365 Plugins 使用。
* 你的租户Microsoft 365，为预配的应用设置所需的回复 URL 和权限，并返回 和 `AppID` `Key`。

使用生成的 和 `AppID` `Key` 在 Microsoft 365 Plugins 设置页中，使用 Microsoft Azure Active Directory (Azure AD) 配置你的将你的一个Microsoft Azure Active Directory (Azure AD) 。
...
1. 执行 PowerShell 脚本：
    ...
    1. 输入应用程序的名称，Microsoft Azure Active Directory (Azure AD) ，例如，将一个或多个 Pluginle 插件。
    1. 输入 Your Your Moodle 服务器的 URL。
    1. 复制 **脚本生成的 (`AppID`)** **应用程序 `Key`** ID () 并保存它们。
...
1. 刷新页面后，可以看到另一个新部分管理员 **同意&其他信息**。
    1. 选择 **"提供管理员同意**"链接，Microsoft 365全局管理员凭据，然后选择"接受"以授予权限。
    1. 在"Microsoft Azure Active Directory (Azure AD) **租户**"字段旁边，选择"检测 **"** 按钮。
    1. 在"OneDrive for Business **URL"旁边**，选择"检测 **"** 按钮。
    1. 填充字段后，再次选择 **"保存更改"** 按钮。

1. 选择" **更新** "按钮以验证安装，然后选择"保存 **更改"**。

1. 在 Your Moodle 服务器和服务器之间Microsoft Azure Active Directory (Azure AD) 。 首先，请执行以下操作：

    > [!NOTE]
    > 根据你的环境，你可以在此阶段选择不同的选项。

1. 在 Your Moodle 服务器和服务器之间Microsoft Azure Active Directory (Azure AD) 。 根据你的环境，你可以在此阶段选择不同的选项。 首先，请执行以下操作：
    1. 切换到"同步 **设置选项卡**。

    1. 在 **"将用户与Microsoft Azure Active Directory (Azure AD)** 同步"部分，选中适用于你的环境的复选框。 您必须选择以下选项：  

        ✔ 在将 Accounts 中的用户创建在一Microsoft Azure Active Directory (Azure AD) 。

        ✔ 为用户更新在 In 一Microsoft Azure Active Directory (Azure AD) 中Microsoft Azure Active Directory (Azure AD) 。

    1. 在"**用户创建限制**"部分，可以设置筛选器以限制Microsoft Azure Active Directory (Azure AD) 用户同步到用户。
    1. "**用户字段映射"** 部分允许您自定义Microsoft Azure Active Directory (Azure AD) 到一个"用户个人资料"字段映射。
    1. 在 **"Teams同步**"部分，可以选择自动创建现有一些或全部的 Teams 课程的组，例如团队。

13. 若要验证 [cron](https://docs.moodle.org/310/en/Cron) 作业并手动运行它们以便首次运行，请选择"将用户与作业同步"部分中的"计划任务管理 **Microsoft Azure Active Directory (Azure AD) 链接。** 这会将您带至 **"计划任务"** 页。

    1. 向下滚动并查找"将用户与 **Microsoft Azure Active Directory (Azure AD)** 同步"作业，然后选择"**立即运行"**。
    1. 如果选择基于现有课程创建组，还可以在作业中运行"创建 **Microsoft 365组"**。
...
适用于教师和学生的免费一Microsoft Teams自动程序可帮助教师和学生回答有关其课程、作业、成绩和其他在一起的问题。 机器人还会向用户内的学生和教师发送一Teams。 自动程序是由 Microsoft 维护的开放源代码项目，可在 [GitHub。](https://github.com/microsoft/Moodle-Teams-Bot)
...
1. 选择" **生成新密码"** 按钮。 复制生成的密码并将其粘贴到"团队密码"页上的"自动程序应用程序密码 **设置** 字段中。
...
> [!div class="checklist"]
> * 选择 **"部署到 Azure**"，然后使用必要的信息完成表单，例如自动程序应用程序 ID、Bot 应用程序密码和 **Teams 设置 密码。** Azure 信息位于"设置 **"** 页面上。 
> * 完成表单后，选中复选框以同意条款和条件。
> * 选择 **"购买"**。 所有 Azure 资源都部署到免费层。

在资源部署到 Azure 后，必须使用消息终结点Microsoft 365一个设置一个将一个"一Microsoft 365一个"设置"插件。 你必须从 Azure 中的机器人获取终结点：
...
在将机器人部署到 Azure 并配置为与你的 Talkle 服务器通信后，你必须部署你的 Microsoft Teams 应用。 为此，你必须加载从上一步中的"Microsoft 365 Plugins Team 设置"页面中下载的应用清单文件。
...
4. 选择之前 `manifest.zip` 下载的程序包，然后选择"保存 **"**。 如果尚未下载应用清单包，可以从在将插件配置页的"team **设置**"选项卡中下载。
...
1. 在"在一个Teams **，** 从你的"Microsoft 365 Plugins 配置"页面打开"一个Microsoft 365一应用"选项卡。

1. 将应用 ID 粘贴到 Microsoft Teams 应用 ID 字段中，并保存更改。

同步一个一流课程后，Microsoft Teams 会自动在团队中安装一个一流应用，在 Teams 的常规频道中创建一个"一般"选项卡，并配置该选项卡以包含用于同步其的一流课程的"一页"。 你现在可以直接从学校开始使用你的Microsoft Teams。
