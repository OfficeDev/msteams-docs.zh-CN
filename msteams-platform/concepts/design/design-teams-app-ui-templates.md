...
可以使用不同的卡片大小 (单个、双倍和完整) 在同一页上堆叠和组织数据可视化效果。 卡片缩放以适应列布局并填充空白。
...
通过筛选器，可以根据所选条件减少你看到的信息。 可以包含包含表、列表、卡片和其他组织内容的组件的筛选器。
...
窗体用于以结构化方式收集、验证和提交用户输入。 清除输入字段的标签和逻辑分组对于良好的用户体验至关重要。
...
任务板（有时称为看板或泳道）是通常用于跟踪工作项或门票状态的卡片的集合。 它还可用于将任何类型的内容排序为类别。 可以在列之间编辑和移动卡片。
