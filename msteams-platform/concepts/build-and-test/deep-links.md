...
你可以创建 Teams 中的信息和功能的链接。 创建深层链接的方案如下所示：
...
> [!NOTE]
>
> 深层链接首先启动浏览器，然后再导航到内容。 Teams 实体上深层链接的行为如下所示：
>
> **选项卡**：  
> ✔直接导航到深层链接 URL。
>
...
>
> **聊天**：  
> ✔文本消息超链接 markdown：直接导航到 deeplink URL。  
> ✔粘贴在常规聊天对话中的链接：直接导航到深层链接 URL。

## <a name="deep-linking-to-your-tab"></a>深入链接你的选项卡

可以在 Teams 中创建实体的深层链接。 这用于创建导航到选项卡中的内容和信息的链接。例如，如果选项卡包含任务列表，团队成员可以创建和共享指向各个任务的链接。 选择链接时，它会导航到焦点位于特定项的选项卡。 若要实现此操作，请向每个项目添加 **复制链接** 操作，以任何最适合 UI 的方式。 当用户执行此操作时，将调用 `shareDeepLink()` 以显示包含用户可复制到剪贴板的链接的对话框。 进行此调用时，你还会传递项目的 ID，当链接被跟踪并重新加载选项卡时，你将返回[上下文](~/tabs/how-to/access-teams-context.md)。

或者，还可以使用本主题后面指定的格式以编程方式生成深层链接。 可以在[自动程序](~/bots/what-are-bots.md)和[连接器](~/webhooks-and-connectors/what-are-webhooks-and-connectors.md)消息中使用深层链接，告知用户有关选项卡或其中项目更改的信息。
...
### <a name="show-a-deep-link-to-an-item-within-your-tab"></a>显示指向选项卡内项的深层链接

若要显示包含选项卡内项的深层链接的对话框，请调用 `microsoftTeams.shareDeepLink({ subEntityId: <subEntityId>, subEntityLabel: <subEntityLabel>, subEntityWebUrl: <subEntityWebUrl> })`。

填写以下字段：

* `subEntityId`：要向其进行深层链接的选项卡内项的唯一标识符。
* `subEntityLabel`：用于显示深层链接的项的标签。
* `subEntityWebUrl`：如果客户端不支持呈现选项卡，则使用回退 URL 的可选字段。
...
导航到深层链接时，Microsoft Teams 只需导航到选项卡，并通过 Microsoft Teams JavaScript 库提供一种机制来检索子实体 ID（如果存在）。

如果选项卡通过深层链接使用，则 [`microsoftTeams.getContext`](/javascript/api/@microsoft/teams-js#getcontext--context--context-----void-) 调用会返回包含 `subEntityId` 字段的上下文。

## <a name="deep-linking-from-your-tab"></a>从选项卡进行深层链接

可以从选项卡深入链接到 Teams 中的内容。如果你的选项卡需要链接到 Teams 中的其他内容（例如频道、消息、另一个选项卡，甚至是打开日程安排对话框），这非常有用。 若要从选项卡触发深层链接，应调用：

```Javascript
microsoftTeams.executeDeepLink(/*deepLink*/);
```

此调用将导航到正确的 URL，或触发客户端操作，例如打开计划或应用安装对话框。 请参阅以下示例：
...
可以通过指定参与者集来创建指向用户间私人聊天的深层链接。 如果与指定参与者不存在聊天，该链接会将用户导航到空的新聊天。 在用户发送第一条消息之前，将在草稿状态下创建新聊天。 否则，可以指定聊天的名称（如果尚不存在）以及应插入到用户撰写框中的文本。 可以将此功能视为用户执行手动操作以导航到或创建聊天，然后键入消息的快捷方式。

例如，如果从机器人以卡片形式返回 Office 365 用户配置文件，则此深层链接可让用户轻松与该用户聊天。
...
* `groupId`：文件的组 ID，ae063b79-5315-4ddb-ba70-27328ba6c31e 

> [!NOTE]
> 可以在通道的 URL 中看到 `threadId` 和 `groupId`。  

机器人、连接器或消息传递扩展卡中使用以下深层链接格式： `https://teams.microsoft.com/l/file/<fileId>?tenantId=<tenantId>&fileType=<fileType>&objectURL=<objectURL>&baseUrl=<baseURL>&serviceName=<Name>&threadId=<threadId>&groupId=<groupId>`
...
## <a name="deep-linking-to-the-scheduling-dialog"></a>计划对话框的深层链接

> [!NOTE]
> 此功能目前以开发人员预览版提供。

可以创建 Teams 内置计划对话框的深层链接。 如果你的应用帮助用户完成日历或计划相关任务，这尤其有用。
...
> [!NOTE]
> 目前不支持指定位置。 必须指定 UTC 偏移量，这意味着生成开始和结束时间时的时区。

若要将此深层链接与机器人配合使用，可以在卡片的按钮中将此链接指定为 URL 目标，或通过 `openUrl` 操作类型点击操作。

## <a name="deep-linking-to-an-audio-or-audio-video-call"></a>深层链接到音频或音频视频通话

可以通过指定呼叫类型来创建仅音频或音频视频调用到单个用户或一组用户的深层链接，如指定为 *audio* 或 *av* 和参与者。 调用深层链接后，在进行呼叫之前，Teams 桌面客户端会提示确认进行呼叫。 对于群组调用，可以在同一深层链接调用中调用一组 VoIP 用户和一组 PSTN 用户。 
