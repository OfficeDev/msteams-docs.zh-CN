...
* 应用通过使用所需参数预先填充深层链接来自动执行或简化某些用户任务，例如创建聊天或安排会议。这可以避免需要用户手动输入信息。
...
可以从选项卡深层链接到 Teams 中的内容。如果选项卡需要链接到 Teams 中的其他内容（例如频道、消息、其他选项卡，或者甚至是打开日程安排对话框），这将非常有用。要从选项卡触发深层链接，请调用：

```Javascript
microsoftTeams.executeDeepLink(/*deepLink*/);
```

此调用将导航到正确的 URL，或触发客户端操作，例如“打开日程安排”或“应用安装”对话框。请参阅以下示例：
...
可以通过指定参与者集来创建指向用户间私人聊天的深层链接。 如果不存在与指定参与者的聊天，该链接会将用户导航到空的新聊天。 在用户发送第一条消息之前，将在草稿状态下创建新聊天。 否则，可以指定聊天的名称（如果尚不存在）以及应插入到用户撰写框中的文本。 可以将此功能视为用户执行手动操作以导航到或创建聊天，然后键入消息的快捷方式。

例如，如果从机器人以卡片形式返回 Office 365 用户配置文件，则用户可通过此深层链接轻松与该用户聊天。
...
可以通过指定呼叫类型来创建仅音频或音频视频调用到单个用户或一组用户的深层链接，如指定为 *audio* 或 *av* 和参与者。 调用深层链接后，在进行呼叫之前，Teams 客户端会提示确认以进行呼叫。 对于群组调用，可以在同一深层链接调用中调用一组 VoIP 用户和一组 PSTN 用户。
