...
必须请求设备权限才能访问本机设备功能。 设备权限对于所有应用构造（如选项卡、任务模块或消息传递扩展）的运行方式类似。 用户必须转到 Teams 设置中的权限页面才能管理设备权限。
...
[Microsoft Teams JavaScript 客户端 SDK](/javascript/api/overview/msteams-client?view=msteams-client-js-latest&preserve-view=true)为 Teams 移动应用提供了访问用户设备权限和[](#manage-permissions)构建更丰富的体验所需的工具。

虽然新式 Web 浏览器中对这些功能的访问是一种标准操作，但你必须通过更新应用清单来通知 Teams 你使用的功能。 此更新允许你在应用在 Teams 桌面客户端上运行时请求权限。
...
用户可以在 Teams 设置中通过选择"允许"**或"拒绝****对特定应用** 的权限"来管理设备权限。
...
> * 通过 `location` [**getLocation API**](/javascript/api/@microsoft/teams-js/location?view=msteams-client-js-latest#getLocation_LocationProps___error__SdkError__location__Location_____void_&preserve-view=true)启用对 的支持。 你必须将此功能用于位置，因为 HTML5 地理位置 API 当前在 Teams 桌面客户端上 `getLocation API` 未完全受支持。
 ...
 * 若要在移动设备上捕获映像，Teams 移动团队会在您调用 时请求权限 `captureImage()` ：
...
* 若要使用相机或访问照片库，Teams 移动在调用 时会询问权限 `selectMedia()` ：
...
* 若要使用麦克风，Teams 移动在调用 时会询问权限 `selectMedia()` ：
...
* 若要提示用户在地图界面上共享位置，Teams 移动会在您调用 时询问权限 `getLocation()` ：
...
将存储每个登录会话的设备权限。 这意味着，如果你登录到另一个 Teams 实例（例如，在另一台计算机中）时，之前会话中的设备权限将不可用。 因此，必须重新同意新会话的设备权限。 这也意味着，如果你注销 Teams 或在 Teams 中切换租户，你的设备权限将从以前的登录会话中删除。  
