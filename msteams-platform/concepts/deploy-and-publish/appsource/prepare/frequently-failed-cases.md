...
ms.locfileid: "52019914"
---
# <a name="tips-for-a-successful-microsoft-teams-app-submission"></a>成功提交 Microsoft Teams 应用的技巧
...
>[!NOTE]
>**[第 1140](/legal/marketplace/certification-policies#1140-teams)** 部分特定于 Microsoft Teams，第 **[1140.4](https://docs.microsoft.com/legal/marketplace/certification-policies#11404-functionality)** 部分介绍 Teams 应用的功能要求。
...
* 对于核心用户方案，该应用不得将用户从 Teams 中退出。 应用中的链接目标不得链接到外部浏览器。 链接目标必须链接到 Teams 中包含的 div 元素，例如任务模块和选项卡。 
* 建议使用任务模块或选项卡向 Teams 中的用户显示信息。
* 所有核心和非核心方案都必须在 Teams 环境中完成，以下方案除外：
  * 隐私策略
  * TOU (使用条款) 
  * 网站链接
  * 注册过程
...
* 注销选项只能让用户注销你的应用的功能，而不是从 Teams 客户端注销。
...
* 频道选项卡不得向应用栏提供左侧栏中与 Teams 主导航发生冲突的图标。
* 选项卡不得在左侧栏中显示应用栏，其图标与 Teams 主导航冲突。
...
* **考虑所有范围**。 请确保当在频道和个人对话中提到 () `@*botname*` 自动程序提供适当的响应。 如果你的机器人在个人或团队范围内未提供有意义的上下文，请通过清单禁用该范围。  (Microsoft Teams `bots` 清单架构参考[](../../../../resources/schema/manifest-schema.md#bots).) 
...
* 支持 iOS 的应用必须在使用最新版本的 iOS 的最新 iPad 设备上完全正常运行。
...
Teams 应用不得向不应查看该数据的受众发布敏感数据，例如信用卡或财务付款方式、个人身份信息 (PIN) 、运行状况或联系人跟踪信息。

### <a name="9989-do-not-transmit-financial-payment-details-or-complete-financial-transactions-via-your-teams-app"></a>&#9989;不要通过 Teams 应用传输财务付款详细信息或完成财务交易
...
* 按照发布者 [证明工作流部分中](/microsoft-365-app-certification/docs/userguide#3publisher-attestation-workflow) 的步骤完成发布者证明过程。 对 appcert@microsoft.com 进行写入。
