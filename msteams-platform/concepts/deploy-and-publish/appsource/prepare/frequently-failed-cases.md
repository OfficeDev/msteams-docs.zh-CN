...
* 应用不得在用户环境中自动下载、安装或启动任何可执行代码。 所有下载都应从用户寻求显式权限。
...
* 确保始终有一种方法可以反向处理以下 (或类似) 行为：
  ...
  * 授权帐户/服务 =>/拒绝帐户/服务。
  * 注册帐户/服务 =>/取消订阅帐户/服务。
* 如果你的应用需要帐户或服务，则必须为用户提供一种注册或创建注册请求的方法。 如果应用需要许可证才能使用，则可能会授予例外。 但是，在此类情况下，必须为新用户注册提供明确的前进方向。
...
* 没有轻松注册流程的应用可能还包括帮助选项卡或指向网页的链接，新用户可在其中查看有关如何使用 Microsoft Teams 配置应用的详细指南。  这是为了确保首次尝试应用时不会阻止新用户。
...
* 用户登录过程必须是配置过程的一部分，并且应在选项卡 UI 中完成。 用户完成配置并加载选项卡后，无需执行进一步操作。
* 不要在登录配置弹出窗口中显示整个网页。
...
[使用调用任务模块的按钮的](/task-modules-and-cards/task-modules/task-modules-bots)自适应卡片可增强机器人用户体验。 与用户键入命令相反，这些卡片和按钮更易于在移动设备上使用。 此外，自动程序响应不应是文本文本和长文本。 机器人必须使用自适应卡片&任务模块，而不是基于对话聊天的用户界面和长文本响应
...
欢迎消息是设置个人/聊天机器人的语气的最佳方法。 这是用户与自动程序的第一次交互。 良好的欢迎消息可以鼓励用户继续探索应用。 如果欢迎或介绍性消息令人困惑或不明确，用户不会立即看到应用的价值，并且会失去兴趣。
请参阅下面的欢迎邮件要求部分。
...
* **使消息简洁且信息丰富**。  很可能，应用的用户体验和知识会有所不同。 用户可能在另一个平台上使用过你的应用，或者对你的应用一无所有。 你想要为所有受众定制消息，用几个句子说明机器人执行哪些操作以及与它交互的方式。 还应说明应用的价值以及用户将如何使用它。
