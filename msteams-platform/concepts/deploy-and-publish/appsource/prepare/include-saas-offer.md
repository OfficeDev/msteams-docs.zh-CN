...
## <a name="manage-license-for-third-party-apps-in-teams"></a>管理 Teams 中第三方应用的许可证

Teams 允许独立软件供应商 (ISV) 管理员或用户管理从 Teams 店面购买的第三方应用的 SaaS 许可证。 ISV 管理员或用户可以轻松分配、取消分配、使用和跟踪 SaaS 许可证。
...
1. [购买后](#post-purchase)
1. [与图形使用权限 API 集成](#integrate-with-graph-usage-right-api)

### <a name="create-an-offer-in-partner-center"></a>在合作伙伴中心创建产品/服务

1. 登录到 [合作伙伴中心](https://partner.microsoft.com/) 并选择 **合作伙伴中心**。

   :::image type="content" source="~/assets/images/first-party-license-mgt/partner-center-home-page.png" alt-text="屏幕截图显示了如何登录到合作伙伴中心帐户。":::

1. 在 **主** 页中，选择 **“市场产品/服务** ”选项卡以定义商业市场产品/服务。

   :::image type="content" source="~/assets/images/first-party-license-mgt/home-page.png" alt-text="屏幕截图显示合作伙伴中心的主页和市场产品/服务选项卡。":::

1. 从左窗格中选择 **“概述** ”。

1. 选择 **“新套餐** > **软件即服务**”。

   :::image type="content" source="~/assets/images/first-party-license-mgt/commercial-marketplace.png" alt-text="屏幕截图显示市场产品/服务页面，可在其中选择新产品/服务。":::

1. 输入 **产品/服务 ID** 和 **产品/服务别名** ，然后选择 **“创建**”。

   > [!NOTE]
   > 如果要创建用于测试的套餐，请将文本 **-ISVPILOT** 添加到产品/服务别名的末尾。 这表示认证团队的产品/服务用于测试目的。 Microsoft 定期使用 **-ISVPILOT** 删除产品/服务。 因此，除了测试许可证管理功能之外，不要使用此标记。

   :::image type="content" source="~/assets/images/first-party-license-mgt/saas.png" alt-text="屏幕截图显示了如何在合作伙伴中心输入套餐 ID 和产品/服务别名。":::

1. 在“产品/服务设置”页中，在设置详细信息下，选中“是”复选框 **，我希望 Microsoft 代表我管理客户许可证**。

   :::image type="content" source="~/assets/images/first-party-license-mgt/saas-isvpilot.png" alt-text="屏幕截图显示了要在 Teams 中为应用管理的设置许可证的产品/服务设置页面。":::

   > [!NOTE]
   >
   > * 这是一次性设置，发布产品/服务后无法对其进行更改。 这允许客户在 Teams 中管理应用的许可证。
   > * 应用清单仅支持应用的一个产品/服务。 为产品/服务中提供的所有计划选择适当的许可证管理解决方案，在产品/服务推送到实时后，无法更改此选项。
...
1. 输入计划 ID 和计划名称，然后选择 **“创建**”。

1. 输入 **计划名称** 和 **计划说明**。

   > [!NOTE]
   > 计划信息显示在 Teams 市场和 [Appsource](https://appsource.microsoft.com/) 的套餐列表 (计划部分) 。

   :::image type="content" source="~/assets/images/first-party-license-mgt/plan-listing.png" alt-text="屏幕截图显示要为应用添加计划名称和计划说明的计划页。":::
...
1. 添加定价和可用性详细信息。
   ...
   :::image type="content" source="~/assets/images/first-party-license-mgt/pricing-availability.png" alt-text="屏幕截图显示了要为应用添加 SaaS 产品/服务的定价和可用性页面。":::

1. 选择 **“保存”草稿**。

1. 选择页面顶部的 **“计划”概述** ，转到显示为此产品/服务创建的所有计划的列表页。

   :::image type="content" source="~/assets/images/first-party-license-mgt/list-of-plans-created.png" alt-text="屏幕截图显示了包含服务 ID、定价模型、可用性、状态和操作的计划列表页。":::

1. 复制创建的计划的服务 ID，以便与 Microsoft Graph 使用权限 API 集成。

### <a name="update-your-teams-app"></a>更新 Teams 应用

更新 Teams 应用以映射到付费功能，并将 [Teams 应用映射](https://aka.ms/TMTG) 到产品/服务并发布。

### <a name="post-purchase"></a>购买后

1. 激活后，客户将从登陆页重定向到 Teams 许可证管理。

1. 成功完成订阅购买后，客户将重定向到应用登陆页以激活订阅。 这是用户 [在 Teams 中购买盈利应用的](https://aka.ms/TMTG)现有体验。

1. 客户在登陆页面激活订阅购买后，客户将通过客户在发布者登陆页上选择的 [重定向 URL](https://teams.microsoft.com/_#/subscriptionManagement) 链接或按钮重定向到 Teams 中的订阅页面。

### <a name="integrate-with-graph-usage-right-api"></a>与图形使用权限 API 集成

与图形使用权限 API 集成，以在应用启动时由拥有购买许可证的客户管理用户权限。 需要使用对使用权限 API 的图形调用来确定用户对应用的权限。

可以调用 Graph API 来确定当前使用计划的有效订阅登录的用户是否有权访问你的应用。 若要调用 Graph UsageRight API 来检查用户权限，请执行以下步骤：
...
1. 调用 UsageRights API 以确定用户具有计划[列表用户使用权限的许可证 - Microsoft Graph beta |Microsoft Docs](/graph/api/user-list-usagerights?view=graph-rest-beta&tabs=http&preserve-view=true)。

   > [!NOTE]
   > 需要具有调用 UsageRights 的最小 `User.Read` 权限。
   > UsageRights API 目前为 beta 版本。 将版本更新到 V1 后，ISV 用户应从 beta 升级到 V1 版本。
...
1. 在报告小组件中选择 **“计划”和“租户** ”以查看月份明智使用情况。
...
选择 **“否”，在这种情况下，我更愿意自行管理客户许可证** 。

有关完整说明，请参阅 [为 SaaS 产品/服务构建登陆页](/azure/marketplace/azure-ad-transactable-saas-landing-page)。

当某人完成为你的应用购买订阅计划，并希望留在 Teams 中，而不将其定向到你的登陆页面时 **，选择“是”，我希望 Microsoft 代表我管理客户许可证**。

有关详细信息，请参阅 [第一方许可证管理](/platform/concepts/deploy-and-publish/appsource/prepare/first-party-license-management)。
