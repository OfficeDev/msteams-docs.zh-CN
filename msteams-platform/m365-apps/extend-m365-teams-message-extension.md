...
若要完成本教程，你需要：
 ...
 - Microsoft Visual Studio可选Teams Toolkit (预览)  (代码) 
...
如果你想要使用示例代码来完成本教程，请按照 [Teams 邮件](https://github.com/microsoft/BotBuilder-Samples/tree/main/samples/javascript_nodejs/50.teams-messaging-extensions-search)扩展搜索示例中的设置步骤快速生成基于Microsoft Teams的邮件扩展。 或者，你可以从针对 [TeamsJS SDK v2 预览](https://github.com/OfficeDev/TeamsFx-Samples/tree/v2/NPM-search-connector-M365)版Teams更新的相同邮件扩展搜索示例开始，然后继续预览 Outlook 中的消息[扩展](#preview-your-messaging-extension-in-outlook)。 更新的示例也可在以下扩展Teams Toolkit *：DevelopmentView* >  *samplesNPM* >  **Search Connector。**
...
你需要使用开发人员预览[](/microsoftteams/platform/resources/schema/manifest-schema-dev-preview) `m365DevPreview` Teams清单架构和清单版本，Teams邮件扩展在 Outlook 中运行。
...
打开Teams应用清单，并更新 `$schema` `manifestVersion` 和，并具有以下值：
...
如果使用 Teams Toolkit创建邮件扩展应用，可以使用它验证对清单文件所做的更改并识别任何错误。 打开命令`Ctrl+Shift+P`调色板并找到"Teams **：** 验证清单文件"或从 Teams Toolkit (的"部署"菜单中选择选项，查找 Teams 左侧的 Visual Studio Code) 。
...
在Microsoft Teams中，消息传递扩展由您托管的 Web 服务和定义 Web 服务的托管位置的应用程序清单组成。 Web 服务利用 [Bot Framework SDK](/azure/bot-service/bot-service-overview) 消息传递架构，并通过为自动程序注册的 Teams 安全通信协议。

若要让用户从 Outlook 与消息扩展进行交互，你需要将Outlook添加到机器人：
...
> [!NOTE]
> 如果使用邮件扩展Teams示例，可以跳过此步骤，[](https://github.com/microsoft/BotBuilder-Samples/tree/main/samples/javascript_nodejs/50.teams-messaging-extensions-search)因为方案不涉及单一Azure Active Directory (AAD) Sign-On身份验证。

Azure Active Directory邮件扩展的 (SSO) 单一登录的工作方式与 Outlook 中的工作方式相同，但是，你需要将多个客户端应用程序标识符添加到租户的应用注册门户中机器人的 [Teams Azure AD](/microsoftteams/platform/bots/how-to/authentication/auth-aad-sso-bots) 应用注册中。
...
1. 使用沙盒租户帐户登录 Teams，并按用户配置文件单击省略号" (**...**) "菜单并打开"关于"，检查"开发人员预览"选项是否处于打开状态，以验证你是否位于"公共 开发者预览版"上。
...
现在可以在桌面和 Web 上测试在 Outlook 中Windows消息扩展。 虽然更新后的邮件扩展将继续在 Teams 中运行，并且具有对邮件扩展的完整[](/microsoftteams/platform/messaging-extensions/what-are-messaging-extensions)功能支持，但在此启用了 Outlook 的体验的早期预览中，请注意以下限制：

* 邮件撰写Outlook邮件撰写[上下文](/microsoftteams/platform/resources/schema/manifest-schema#composeextensions)。 即使你的邮件Teams包括`commandBox`  `compose` 在其清单中的上下文，当前预览仅限于邮件组合 () 选项。 不支持从全局搜索Outlook *消息* 扩展。
* [Outlook](/microsoftteams/platform/messaging-extensions/how-to/action-commands/define-action-command?tabs=AS) 中不支持基于操作的邮件扩展Outlook。 如果你的应用同时具有基于搜索和基于操作的命令，它将Outlook但操作菜单将不可用。
...
Outlook和Office用户的个人选项卡可以通过以下三种方式之一跨测试 (或生产) 租户分发给预览受众：
...
作为全局管理员，你可以从 上传并预安装应用包 https://admin.microsoft.com/。 有关详细信息[，请参阅Microsoft 365 应用版集成应用](/microsoft-365/admin/manage/test-and-deploy-microsoft-365-apps)门户中的测试并部署合作伙伴部署解决方案。
