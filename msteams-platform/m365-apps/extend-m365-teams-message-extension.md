...
> [!NOTE]
> *目前Teams开发人员预览版* Microsoft 365扩展跨应用程序扩展 [邮件。](../resources/dev-preview/developer-preview-intro.md) 预览版中包含的功能可能不完整，在公开发布之前可能会发生更改。 它们仅供测试和探索使用。 它们不应在生产应用程序中使用。

基于搜索[的邮件扩展](/microsoftteams/platform/messaging-extensions/what-are-messaging-extensions)允许用户搜索外部系统并通过客户端的撰写邮件区域共享Microsoft Teams结果。 通过跨[Teams](overview.md)预览) 扩展 Microsoft 365 (应用，现在可以将基于搜索的 Teams 邮件扩展Outlook Windows 桌面和 Web 体验。

更新基于搜索的邮件扩展Teams以运行Outlook包括以下步骤：
...
若要使用示例代码完成本教程，请按照[Teams 邮件](https://github.com/microsoft/BotBuilder-Samples/tree/main/samples/javascript_nodejs/50.teams-messaging-extensions-search)扩展搜索示例中的设置步骤快速生成基于Microsoft Teams的邮件扩展。 或者，你可以从为[TeamsJS SDK v2](https://github.com/OfficeDev/TeamsFx-Samples/tree/v2/NPM-search-connector-M365)预览Teams更新的相同邮件扩展搜索示例开始，然后继续预览 Outlook 中[的消息扩展](#preview-your-message-extension-in-outlook)。 更新的示例也可在扩展中  >  *Teams Toolkit：Development View samples*  >  **NPM Search Connector**。
...
如果使用 Teams Toolkit创建邮件扩展应用，可以使用它验证对清单文件所做的更改并识别任何错误。 打开命令调色板并找到"Teams：验证清单文件"，或从 Teams Toolkit (的"部署"菜单中选择选项，查找 Teams 左侧 `Ctrl+Shift+P` 的 Visual Studio Code) 。 
...
1. 将应用程序Teams打包[ (包，](/microsoftteams/platform/resources/schema/manifest-schema#icons)将应用图标) 压缩文件中。 如果你使用Teams Toolkit创建应用，可以使用以下项的"部署"菜单中的 **Zip Teams 元数据** 包选项轻松Teams Toolkit： 
...
1. 使用沙盒租户帐户登录 Teams，然后通过按用户配置文件单击省略号 ( **...**) 菜单并打开"关于"来检查"开发人员预览"选项是否处于打开状态，以验证你是否位于公共开发者预览版 上。 
...
现在可以在桌面和 Web 上的 Outlook 中测试Windows扩展。 虽然更新后的邮件扩展将继续在 Teams 中运行，并且具有对邮件[](/microsoftteams/platform/messaging-extensions/what-are-messaging-extensions)扩展的完整功能支持，但在此启用了 Outlook 的体验的早期预览中应注意以下限制：

* 邮件撰写Outlook邮件[撰写上下文。](/microsoftteams/platform/resources/schema/manifest-schema#composeextensions) 即使您的邮件Teams包括在其清单中作为上下文，当前预览也仅限于邮件组合 () `commandBox`  `compose` 选项。 不支持从全局搜索Outlook *消息* 扩展。
...
Outlook启用Teams邮件扩展是预览版，不支持用于生产用途。 下面将了解如何分发启用Outlook邮件扩展以预览访问群体，以便进行测试。
