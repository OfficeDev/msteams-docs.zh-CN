...
若要完成本教程，你需要：
 ...
 - Microsoft Visual Studio"预览"Teams Toolkit (扩展) 代码 (可选) 
...
如果要使用示例代码完成本教程，请按照 [Teams 邮件](https://github.com/microsoft/BotBuilder-Samples/tree/main/samples/javascript_nodejs/50.teams-messaging-extensions-search)扩展搜索示例中的设置步骤快速生成基于Microsoft Teams的邮件扩展。 或者，你可以从为 [TeamsJS SDK v2 预览](https://github.com/OfficeDev/TeamsFx-Samples/tree/v2/NPM-search-connector-M365)Teams更新的相同邮件扩展搜索示例开始，然后继续预览 Outlook 中[的消息扩展](#preview-your-messaging-extension-in-outlook)。 更新的示例也可在以下扩展Teams Toolkit *：DevelopmentView* >  *samplesNPM* >  **Search Connector。**
...
1. 确认你的Outlook频道已与Microsoft Teams频道 **窗格中的频道** 一起列出：
    ...
    :::image type="content" source="images/azure-bot-channels.png" alt-text="Azure 自动程序频道窗格，Microsoft Teams Outlook频道":::

## <a name="update-azure-ad-app-registration-for-sso"></a>更新Azure AD SSO 的应用注册

> [!NOTE]
> 如果使用的是邮件扩展Teams示例，可以跳过此步骤[](https://github.com/microsoft/BotBuilder-Samples/tree/main/samples/javascript_nodejs/50.teams-messaging-extensions-search)，因为方案不涉及单一Azure Active Directory (AAD) Sign-On身份验证。

Azure Active Directory邮件扩展的 (SSO) 单一登录的工作方式与[在 Outlook](/microsoftteams/platform/bots/how-to/authentication/auth-aad-sso-bots) 中相同，但在 Teams 中，你需要将多个客户端应用程序标识符添加到租户的应用注册门户中自动程序的 Azure AD 应用注册中。
...
1. 将应用程序Teams打包[ (包，](/microsoftteams/platform/resources/schema/manifest-schema#icons)将应用图标) 压缩文件中。 如果使用Teams Toolkit创建应用，则通过使用以下项的"部署"菜单中的 **Zip Teams 元数据** 包选项，可以轻松Teams Toolkit：
...
现在可以在桌面和 Web 上的 Outlook 中测试Windows扩展。 虽然更新后的邮件扩展将继续在 Teams 中运行，并且具有对邮件扩展的完整[](/microsoftteams/platform/messaging-extensions/what-are-messaging-extensions)功能支持，但在此启用了 Outlook 的体验的早期预览中，请注意以下限制：
...
若要预览在桌面Outlook Windows中运行的应用，Outlook使用测试租户的凭据登录。 单击" **新建电子邮件"**。 打开 **顶部功能区** 上的"更多应用"弹出菜单。 将列出你的邮件扩展。 你可以从该调用调用，并使用它，就像在邮件中撰写Teams。

### <a name="outlook-on-the-web"></a>Outlook 网页版

若要预览在 Outlook 网页版 中运行的应用，[outlook.com 使用测试](https://www.outlook.com)租户的凭据登录。 单击" **新建邮件"**。 打开 **合成窗口** 底部的"更多应用"弹出菜单。 将列出你的邮件扩展。 你可以从该调用调用，并使用它，就像在邮件中撰写Teams。
