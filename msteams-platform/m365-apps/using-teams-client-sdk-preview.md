...
若要在 Outlook 和 Office 中运行，你的应用将需要依赖 [npm](https://www.npmjs.com/package/@microsoft/teams-js/v/2.0.0-beta.1) `@microsoft/teams-js@2.0.0-beta.1` 程序包 (或更高版本的) 。 若要手动执行这些步骤，以及 API 更改详细信息，请参阅以下有关转换为承诺的回调和组织为功能的 [](#callbacks-converted-to-promises) [API 的部分](#apis-organized-into-capabilities)。
...
命名空间`app`包含整个应用使用情况所需的顶级 API，包括Microsoft Teams、Office和Outlook。 来自各种其他 TeamsJS `app` 命名空间的所有 API 已移动到 TeamsJS SDK v2 Preview 中的命名空间：
...
命名空间`pages`包括用于在各种客户端（包括 Microsoft 365、Teams、Office 和 Outlook）中运行和导航网页Outlook。 它还包括多个子功能，作为子命名空间实现。
...
该`Context`接口已移动到`app`命名空间并进行了更新，以对类似的属性进行分组，以在 Outlook 和 Office 中运行时获得更好的可伸缩性，Teams。

添加了一个新 `app.Context.app.host.name` 属性，使个人选项卡能够根据主机应用程序区分用户体验。

还可以查看 TeamsJS SDK v2 预览  [`transformLegacyContextToAppContext`](https://github.com/OfficeDev/microsoft-teams-library-js/blob/2.0-preview/packages/teams-js/src/public/app.ts) 源中的 函数，直观呈现更改。
