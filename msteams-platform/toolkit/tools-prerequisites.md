---
title: 使用 Visual Studio Code 创建 Teams 应用的先决条件
author: zyxiaoyuer
...
| &nbsp; | 基本要求 | 使用| 对于环境类型|
   ...
   | &nbsp; | [Microsoft Teams](https://www.microsoft.com/microsoft-teams/download-app) | 通过聊天、会议和通话应用与你一起工作的每个人协作 - 一切尽在一个位置。| JavaScript 和 SPFx|
   | &nbsp; | [Node.js](https://nodejs.org/en/download/) | 后端 JavaScript 运行时环境。 使用最新的 v16 LTS 版本。| JavaScript 和 SPFx|
   | &nbsp; |[节点包管理器 (NPM) ](https://www.npmjs.com/package/@microsoft/teamsfx) | 安装和管理在 Node.js 和 ASP.NET Core 应用程序中使用的包。| JavaScript 和 SPFx|
   | &nbsp; | [微软&nbsp;Edge (建议](https://www.microsoft.com/edge)) 或 [Google Chrome](https://www.google.com/chrome/) | 包含开发人员工具的浏览器。 | JavaScript 和 SPFx|
   ...
   | **可选** | &nbsp; | &nbsp; | &nbsp; |
   | &nbsp; | [Azure Tools for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-node-azure-pack) 和 [Azure CLI](/cli/azure/install-azure-cli) | 访问存储的数据或在 Azure 中为 Teams 应用部署基于云的后端。 | JavaScript|
   | &nbsp; | [React Chrome 开发人员工具](https://chrome.google.com/webstore/detail/react-developer-tools/fmkadmapgofadopljbjfkapdkoienihi)或 [microsoft Edge React开发人员工具&nbsp;](https://microsoftedge.microsoft.com/addons/detail/react-developer-tools/gpphkfbcpidddadnkolkpfckpihlkkil) | 开放源代码React JavaScript 库的浏览器 DevTools 扩展。 | JavaScript|
   ...
   > [!NOTE]
   >
   > * 本文档使用 Teams 工具包版本 4.0.0 和 Nodejs 版本 16 进行测试。
   > * 为 Microsoft Graph 资源管理器添加书签，了解 Microsoft Graph 服务。 此基于浏览器的工具允许你在应用外部查询和访问 Microsoft Graph。

## <a name="accounts-to-build-your-teams-app"></a>用于生成 Teams 应用的帐户

在开始生成 Teams 应用之前，请确保具有以下帐户：

| 帐户 | 使用| 对于环境类型|
| --- | --- |
|[具有有效订阅的 Microsoft 365 帐户](#microsoft-365-developer-program)|开发应用时的 Teams 开发人员帐户。| JavaScript 和 SPFx|
|[Azure 帐户](#azure-account)|Azure 上的后端资源。| JavaScript 和 SPFx|
...
* **Microsoft 商业版工作帐户**
  ...
  :::row:::
    ...
    :::column span="3":::
       ...
       该帐户提供对所有小型、中型和企业业务级 Microsoft 云服务的访问权限。 服务包括 Azure、Microsoft Intune或 Microsoft 365。 
...
在 Visual Studio Code 中使用 Teams 工具包，需要一个 Azure 帐户来托管 Teams 应用或 Teams 应用的后端资源。 在以下情况下，必须使用 Azure 订阅：

* 如果已在 Azure 以外的其他云提供商上拥有现有应用，并且想要在 Teams 平台上集成该应用，则必须拥有 Azure 订阅。
* 可以选择 Azure 订阅来使用其他云提供商托管后端资源，或者在自己的服务器上托管后端资源（如果这些资源可从公共域使用）。
...
使用 SPFx 环境创建 Teams 应用时，需要在部署时使用 SharePoint 集合网站管理员帐户进行托管。 如果你使用的是 Microsoft 365 开发人员计划租户，则可以使用当时创建的管理员帐户。
...
1. 从 Teams 工具包 :::image type="icon" source="../assets/images/teams-toolkit-v2/teams-toolkit-sidebar-icon.PNG"::: 工具栏中选择“Teams 工具包”图标。

   > [!NOTE]
   > 如果看不到 选项，请参阅[在 Visual Studio Code 中安装 Teams 工具包](install-Teams-Toolkit.md)以安装 Teams 工具包扩展。
1. 在“**帐户**”下 **选择“登录到 M365**”，然后登录到 Microsoft 365 帐户。
...
如果你有管理员权限，请执行以下步骤，使用管理中心上传自定义应用：
  ...
  1. 选择“ :::image type="icon" source="../assets/images/teams-toolkit-v2/showall icon.PNG"::: > **Teams 应用** > **设置策略”** 图标。
  ...
  5. 选择 **保存**。
     ...
     现在，请确保使用 Visual Studio Code [或 Teams 客户端验证旁加载权限中所述的步骤拥有旁加载权限。](#sideloading-permission)
