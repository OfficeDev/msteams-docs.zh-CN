...
- 如果项目已安装 `botbuilder` 相关 [包作为](https://github.com/Microsoft/botbuilder-js#packages) 依赖项，请确保它们的版本相同且版本为 `>= 4.9.3` 。  ([问题 - 所有 BOTBUILDER](https://github.com/BotBuilderCommunity/botbuilder-community-js/issues/57#issuecomment-508538548) 程序包的版本应) 
...
若要创建用于访问 Microsoft Graph API 的图形客户端对象，将需要凭据进行身份验证。 SDK 提供了几个凭据类，可供选择以满足各种要求。 您需要先加载配置，然后再使用任何凭据。

- 在浏览器环境中，需要显式传递配置参数。 基架项目React提供了环境变量来使用。
...
Microsoft 365租户凭据无需与应用Teams交互。 你可以将 Microsoft Graph为应用程序。
...
凭据类实现在 Azure 库 API `TokenCredential` 中广泛使用的接口。 它们旨在为特定范围提供访问令牌。 在某些方案中，以下凭据类表示不同的标识：
...
与机器人相关的类存储在自动 [程序文件夹 下](https://github.com/OfficeDev/TeamsFx/tree/main/packages/sdk/src/bot)。

`TeamsBotSsoPrompt` 可以与 Bot 框架集成。 它简化了开发自动程序应用程序的身份验证过程。

### <a name="helper-functions"></a>帮助程序函数

TeamsFx SDK 提供了帮助程序函数，以便于第三方库的配置。 它们位于核心文件夹 [下](https://github.com/OfficeDev/TeamsFx/tree/main/packages/sdk/src/core)。
